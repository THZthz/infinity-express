...
using vec2i64 = glm::vec<2, int64_t>;
using vec2u = glm::vec<2, uint32_t>;
using vec2u64 = glm::vec<2, uint64_t>;
...
static inline bool ApproxEqual(const vec2 &a, const vec2 &b, float epsilon = 1e-6f) { return ApproxEqual(a.x, b.x, epsilon) && ApproxEqual(a.y, b.y, epsilon); }
...
static inline vec2 MulAddV(const vec2 &a, float s, const vec2 &b) { /*a + s * b*/return {a.x + s * b.x, a.y + s * b.y}; }
static inline vec2 MulSubV(const vec2 &a, float s, const vec2 &b) { /*a - s * b*/return {a.x - s * b.x, a.y - s * b.y}; }
static inline vec2 MulSV(float s, const vec2 &v) { return {s * v.x, s * v.y}; }
static inline float LenV(const vec2 &v) { return Sqrt(v.x * v.x + v.y * v.y); }
static inline vec2 LenV(const vec2 &v, float length) { return MulSV(length / LenV(v), v); }
static inline float DistV(const vec2 &a, const vec2 &b) { return LenV(SubV(a, b)); }
static inline float DotV(const vec2 &a, const vec2 &b) { return a.x * b.x + a.y * b.y; }
static inline float DistSqV(const vec2 &a, const vec2 &b) { vec2 c = SubV(a, b); return DotV(c, c); }
static inline vec2 NormV(const vec2 &v) { float length = LenV(v); if (length < FLT_EPSILON) return {0, 0}; float invLength = 1.0f / length; return {invLength * v.x, invLength * v.y}; }
static inline void NormV(vec2 *v) { float length = LenV(*v); if (length < FLT_EPSILON) *v = {0, 0}; float invLength = 1.0f / length; *v = {invLength * v->x, invLength * v->y}; }
static inline vec2 NormV(const vec2 &v, float &l) { l = LenV(v); if (l < FLT_EPSILON) return {0, 0}; float invLength = 1.0f / l; return {invLength * v.x, invLength * v.y}; }
static inline vec2 CrossSV(float s, const vec2 &v) { return {-s * v.y, s * v.x}; }
static inline vec2 CrossVS(const vec2 &v, float s) { return {s * v.y, -s * v.x}; }
static inline float CrossV(const vec2 &a, const vec2 &b) { return a.x * b.y - a.y * b.x; }
static inline vec2 AbsV(const vec2 &a) { return {Abs(a.x), Abs(a.y)}; }
static inline vec2 MinV(const vec2 &a, const vec2 &b) { return {Min(a.x, b.x), Min(a.y, b.y)}; }
static inline vec2 MaxV(const vec2 &a, const vec2 &b) { return {Max(a.x, b.x), Max(a.y, b.y)}; }
static inline vec2 ClampV(const vec2 &a, const vec2 &lo, vec2 hi) { return MaxV(lo, MinV(a, hi)); }
static inline vec2 ScaleV(const vec2 &a, float b) { return {a.x * b, a.y * b}; }
static inline vec2 DivV(const vec2 &a, float b) { return ScaleV(a, 1.0f / b); }
static inline vec2 SkewV(const vec2 &a) { return {-a.y, a.x}; }
static inline vec2 CCW90V(const vec2 &a) { return {a.y, -a.x}; }
static inline float Det2(const vec2 &a, const vec2 &b) { return a.x * b.y - a.y * b.x; }
static inline bool ParallelV(const vec2 &a, const vec2 &b, float kTol) { float k = LenV(a) / LenV(b); vec2 nb = ScaleV(b, k); if (Abs(a.x - nb.x) < kTol && Abs(a.y - nb.y) < kTol) return true; return false; }
static inline float ProjectV(const vec2 &left, const vec2 &right) { return DotV(left, right) / DotV(right, right); }
static inline float AngleV(const vec2 &a, const vec2 &b) { return Acos(DotV(a, b) / (LenV(a) * LenV(b))); }
