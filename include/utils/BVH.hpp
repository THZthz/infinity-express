...
public:
	...
	/// When querying functions met overlapping AABB, the responding proxy will be called
	/// with the function.
	/// \return NVG_TRUE if the query should continue.
	using TreeQueryCallback = bool (*)(int32_t proxyId, void *userData, void *context);

	/// When ray hits a AABB box, this function is called.
...
public:
	...
	int32_t Add(AABB aabb, uint32_t categoryBits, void *userData);
	...
	bool Move(int32_t proxyId, AABB aabb1);

	/// Query an AABB for overlapping proxies. The callback class
	/// is called for each proxy that overlaps the supplied AABB.
	void
	QueryFiltered(AABB aabb, uint32_t maskBits, TreeQueryCallback callback, void *context);

	/// Query an AABB for overlapping proxies. The callback class
	/// is called for each proxy that overlaps the supplied AABB.
	void Query(AABB aabb, TreeQueryCallback callback, void *context);
	...
	/// Render the enlarged (fat) AABB for a proxy.
	AABB GetFatAABB(int32_t proxyId);
