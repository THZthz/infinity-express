...
void
nvgBeginFrame(NVGcontext* ctx, float windowWidth, float windowHeight, float devicePixelRatio)
{
	ctx->nstates = 0;
	nvgSave(ctx);
	nvgReset(ctx);
	...
	ctx->params
	    .renderViewport(ctx->params.userPtr, windowWidth, windowHeight, devicePixelRatio);
}
...
float
nvgSRGBtoLinear(unsigned char c)
{
	//float c = srgb / 255.0f;
...
void
nvgFillRule(NVGcontext* ctx, int rule)
{
	NVGstate* state = nvg__getState(ctx);
	state->fillRule = rule;
}
...
void
nvgDashArray(NVGcontext* ctx, float* dashes)
{
	NVGstate* state = nvg__getState(ctx);
...
void
nvgDashOffset(NVGcontext* ctx, float offset)
{
	NVGstate* state = nvg__getState(ctx);
...
void
nvgSetTransform(NVGcontext* ctx, float a, float b, float c, float d, float e, float f)
{
	NVGstate* state = nvg__getState(ctx);
...
void
nvgTransform(NVGcontext* ctx, const float* mat)
{
	nvgSetTransform(ctx, mat[0], mat[1], mat[2], mat[3], mat[4], mat[5]);
}
...
int
nvgCreateImageMem(NVGcontext* ctx, int imageFlags, unsigned char* data, int ndata)
{
	int w, h, n, image;
...
int
nvgCreateImageRGBA(NVGcontext* ctx, int w, int h, int imageFlags, const unsigned char* data)
{
	return ctx->params
	    .renderCreateTexture(ctx->params.userPtr, NVG_TEXTURE_RGBA, w, h, imageFlags, data);
}
...
nvg__appendCommands(NVGcontext* ctx, float* vals, int nvals)
{
	...
	while (i < nvals)
	{
		...
		switch (cmd)
		{
			case NVG_MOVETO:
				nvgTransformPoint(
				    &vals[i + 1], &vals[i + 2], state->xform, vals[i + 1], vals[i + 2]);
				i += 3;
				break;
			case NVG_LINETO:
				nvgTransformPoint(
			...
			case NVG_BEZIERTO:
				nvgTransformPoint(
				    &vals[i + 1], &vals[i + 2], state->xform, vals[i + 1], vals[i + 2]);
				nvgTransformPoint(
				    &vals[i + 3], &vals[i + 4], state->xform, vals[i + 3], vals[i + 4]);
				nvgTransformPoint(
				    &vals[i + 5], &vals[i + 6], state->xform, vals[i + 5], vals[i + 6]);
				i += 7;
				break;
			case NVG_CLOSE: i++; break;
			case NVG_WINDING: i += 2; break;
			default: // NVG_RESTART
				i++;
		}
	}
...
static void
nvg__vset2(NVGvertex* vtx, float x0, float y0, float x1, float y1, float u, float v)
{
	vtx->x0 = x0;
	vtx->y0 = y0;
	vtx->x1 = x1;
...
static NVGvertex*
nvg__segment(NVGvertex* vtx, float x0, float y0, float x1, float y1)
{
    ...
    int level) //, int type)
{
	...
	{ //if (level < 9)  -- this was the previous logic (so if level reached 10, no point was added!)
	...
	while (i < ctx->ncommands)
	{
		...
		switch (cmd)
		{
			...
			case NVG_LINETO:
				...
				nvg__addPoint(
				    ctx, p[0],
				    p[1]); // note that p is not added if equal to previous point (w/in distTol)
				i += 3;
				break;
			case NVG_BEZIERTO:
				...
				if (last != NULL)
				{
					...
					p = &ctx->commands[i + 5];
					nvg__tesselateBezier(
					    ctx, last->x, last->y, cp1[0], cp1[1], cp2[0], cp2[1], p[0], p[1], 0);
				}
				i += 7;
				break;
			case NVG_CLOSE:
				path = nvg__lastPath(ctx);
				if (path) path->closed = 1;
				i++;
				break;
			case NVG_WINDING:
				path = nvg__lastPath(ctx);
				if (path) path->winding = (unsigned char)ctx->commands[i + 1];
				i += 2;
				break;
			case NVG_RESTART:
				path = nvg__lastPath(ctx);
				if (path) path->restart = 1;
				i++;
				break;
			default: i++;
		}
...
static void
nvg__calcBounds(NVGcontext* ctx)
{
	NVGpathCache* cache = ctx->cache;
	int i, j;
    ...
    int ncap)
{
	...
	if (a1 > a0)
		a1 -= NVG_PI *
		      2; // enforce CCW sweep (negative sweep angle in screen coords, where y is down)
	...
	for (i = 0; i < n; i++)
	{
		...
		if (i > 0) dst = nvg__segment(dst, ax, ay, rx, ry);
		ax = rx;
		ay = ry;
	}
	return dst;
    ...
    float miterLimit)
{
	...
	float mlimsq =
	    w * w * miterLimit * miterLimit; // (miter_length_limit/2)^2, since w = stroke_width/2
	...
	for (i = 0; i < cache->npaths; ++i)
	{
		...
		else csegs += (path->count * 2 + 1) * 2; // plus one for loop
	}
	...
	for (i = 0; i < cache->npaths; ++i)
	{
		...
		else if (path->count == 1)
		{
			...
			if (lineCap == NVG_BUTT) continue;
			p0 = &pts[0];
			temppt.x = p0->x + w / 256;
			temppt.y = p0->y; // offset slightly so calc of normal vector works
			p1 = &temppt;
		}
		else
		{
			...
			p1 = &pts[1];
			p2 =
			    &pts[2]; // never used if count == 2, although we should probably still set to NULL
		}
		...
		d01y = p1->y - p0->y;
		n01x = -d01y;
		n01y = d01x; // left normal to segment - +90 deg CCW rotation
		...
		if (closed == 0)
		{
			...
			ry = p0->y - w * n01y;
			// start cap
			if (lineCap == NVG_BUTT) dst = nvg__segment(dst, rx, ry, lx, ly);
		...
		for (j = closed ? 0 : 2; j < path->count; ++j)
		{
			...
			float _len = nvg__normalize(
			    &n12x, &n12y); // throwaway so we don't need separate decl. for vars below

			// these are only needed for miter join
			float miter_denom =
			    nvg__maxf(1e-6f, 1 + (n01x * n12x + n01y * n12y)); // 1 + dot(n01,n12)
			...
			int join =
			    (lineJoin == NVG_MITER && mlensq <= mlimsq && miter_denom > 1e-6f)
			        ? NVG_MITER
			        : NVG_BEVEL;
			...
			if (j > 0)
				dst = nvg__segment(
				    dst, lx, ly, l01x, l01y); // first segment (and only for miter join)
			else
			{
				l00x = l01x;
				l00y = l01y;
			...
			else if (ljoin == NVG_BEVEL) dst = nvg__segment(dst, l01x, l01y, l12x, l12y);
			lx = l12x;
			ly = l12y;
			...
			if (j > 0)
				dst = nvg__segment(
				    dst, r01x, r01y, rx, ry); // first segment (and only for miter join)
			else
			{
				r00x = r01x;
				r00y = r01y;
			...
			else if (rjoin == NVG_BEVEL) dst = nvg__segment(dst, r12x, r12y, r01x, r01y);
			rx = r12x;
			ry = r12y;
		...
		if (closed == 0)
		{
			...
			// end cap
			if (lineCap == NVG_BUTT) dst = nvg__segment(dst, lx, ly, rx, ry);
			else if (lineCap == NVG_SQUARE)
			{
				float cdx = n01y * w, cdy = -n01x * w; // = w*normalized(d01);
...
static void
nvg__dashStroke(NVGcontext* ctx, float scale, float strokeWidth)
{
	NVGstate* state = nvg__getState(ctx);
	NVGpathCache* cache = ctx->cache;
	...
	for (j = 0; j < ndashes; j++) allDashLen += dashes[j];
	if (ndashes & 1) allDashLen *= 2.0f;
	...
	for (i = 0; i < npaths; ++i)
	{
		...
		for (j = 1; j < jlim;)
		{
			...
			if (totalDist + dist > dashLen)
			{
				...
				float x = cur.x + dx * d;
				float y = cur.y + dy * d;
				if (!dashState) nvg__addPath(ctx); // starting dash
			...
			else
			{
				totalDist += dist;
				cur = points[k];
				if (dashState) nvg__addPoint(ctx, cur.x, cur.y);
				j++;
			}
		}
	}
...
nvg__expandFill(NVGcontext* ctx) //, float w, int lineJoin, float miterLimit)
{
	...
	if (!nvg__getState(ctx)->shapeAntiAlias && cache->npaths == 1 && cache->paths[0].nfill > 2)
	{
		...
		for (i = 2; i < path->nfill - 1 && path->convex; ++i)
		{
			v = &path->fill[i];
			path->convex =
			    (nvg__triarea2(v0->x0, v0->y0, v->x0, v->y0, v->x1, v->y1) > 0) == sgn;
		}
	}
	return 1;
}
...
nvgArc(NVGcontext* ctx, float cx, float cy, float r, float a0, float a1, int dir)
{
	...
	if (dir == NVG_CW)
	{
		if (nvg__absf(da) >= NVG_PI * 2) { da = NVG_PI * 2; }
		else
		{
...
{
	float vals[] = {
	    NVG_MOVETO, x,     y,          NVG_LINETO, x, y + h,    NVG_LINETO,
...
nvgFill(NVGcontext* ctx)
{
	...
	int flags =
	    (state->shapeAntiAlias ? 0 : NVG_PATH_NO_AA) |
	    (state->fillRule == NVG_NONZERO ? 0 : NVG_PATH_EVENODD);
	...
	ctx->params.renderFill(
	    ctx->params.userPtr, &fillPaint, state->compositeOperation, &state->scissor, flags,
...
nvgStroke(NVGcontext* ctx)
{
	...
	float strokeWidth =
	    nvg__maxf(state->strokeWidth * scale, 0.0f); //nvg__clampf(..., 200.0f);
	NVGpaint strokePaint = state->stroke;
	NVGpath* paths0 = NULL;
	int i, npaths0, npoints0;
	int flags =
	    (state->shapeAntiAlias
	         ? 0
	         : NVG_PATH_NO_AA); // stroke fill always uses non-zero fill rule
	...
	ctx->params.renderFill(
	    ctx->params.userPtr, &strokePaint, state->compositeOperation, &state->scissor, flags,
...
// Add fonts
int
nvgCreateFont(NVGcontext* ctx, const char* name, const char* path)
{
...
int
nvgAddFallbackFont(NVGcontext* ctx, const char* baseFont, const char* fallbackFont)
{
...
void
nvgFontHeight(NVGcontext* ctx, float height)
{
	NVGstate* state = nvg__getState(ctx);
...
nvgFontFaceId(NVGcontext* ctx, int font)
{
	...
	if (fonsSetFont(ctx->fs, font) == FONS_INVALID) return -1;
	state->fontId = font;
	return 0;
}
...
void
nvgAtlasTextThreshold(NVGcontext* ctx, float px)
{
...
nvg__renderText(NVGcontext* ctx, NVGvertex* verts, int nverts)
{
	...
	ctx->params.renderTriangles(
	    ctx->params.userPtr, &paint, state->compositeOperation, &state->scissor, verts,
	    nverts);
}

static void
nvg__fonsSetup(NVGcontext* ctx, float scale)
{
	NVGstate* state = nvg__getState(ctx);
	fonsSetFont(ctx->fs, state->fontId);
...
static float
nvg__textAsPaths(NVGcontext* ctx, float x, float y, const char* string, const char* end)
{
	NVGstate* state = nvg__getState(ctx);
	FONStextIter iter;
	FONSquad q;
	...
	while (fonsTextIterNext(ctx->fs, &iter, &q))
	{
		...
		for (int i = 0; i < n_points; i++)
		{
			if (points[i].type == STBTT_vmove)
			{
				...
				{
					float restart[] = {
					    NVG_RESTART}; // flag indicating start of new path (and not just subpath)
					nvg__appendCommands(ctx, restart, 1);
				}
			}
			else if (points[i].type == STBTT_vline) nvgLineTo(ctx, points[i].x, points[i].y);
				...
				nvgBezierTo(
				    ctx, points[i].cx, points[i].cy, points[i].cx1, points[i].cy1, points[i].x,
				    points[i].y);
		}
...
static float
nvg__textFromAtlas(NVGcontext* ctx, float x, float y, const char* string, const char* end)
{
	NVGstate* state = nvg__getState(ctx);
...
float
nvgTextAsPaths(NVGcontext* ctx, float x, float y, const char* string, const char* end)
{
	nvg__fonsSetup(ctx, 1.0f);
...
struct RpPoint
ToRpPoint(float px, float py, float ppx, float ppy)
{
	struct RpPoint v;

	// Convert points to a line with len and normalized x,y as vec
	v.x = ppx - px;
	v.y = ppy - py;
	v.len = nvg__sqrtf(v.x * v.x + v.y * v.y); // Length of vec normalized
	v.nx = v.x / v.len;
	v.ny = v.y / v.len;
	v.ang = nvg__atan2f(v.ny, v.nx); // Direction of vec

	return v;
}

void
nvgSegment(NVGcontext* ctx, float x1, float y1, float x2, float y2)
{
	nvgMoveTo(ctx, x1, y1);
	nvgLineTo(ctx, x2, y2);
}
...
void
nvgCapsule(NVGcontext* ctx, float x1, float y1, float x2, float y2, float r)
{
	...
	if (r <= FLT_EPSILON)
	{
		nvgSegment(ctx, x1, y1, x2, y2);
		return;
	}
	if (len2 <= FLT_EPSILON)
	{
		nvgCircle(ctx, x1, y1, r);
		return;
	}

	float px = -dy, py = dx; // Perpendicular vector.
	float ang = atan2f(py, px);

	// Draw.
	nvgArc(ctx, x2, y2, r, ang, ang + NVG_PI, NVG_CW);
	nvgArc(ctx, x1, y1, r, ang + NVG_PI, ang, NVG_CW);
}
...
void
nvgPolygon(NVGcontext* ctx, int count, float* vertices_)
{
	if (count < 2) return;
	struct Pointf* vertices = (struct Pointf*)vertices_;
	nvgMoveTo(ctx, vertices[0].x, vertices[0].y);
	for (int i = 1; i < count; i++) nvgLineTo(ctx, vertices[i].x, vertices[i].y);
	nvgClosePath(ctx);
}

void
nvgRoundPolygon(NVGcontext* ctx, int count, float* verts_, float r)
{
	struct Pointf* verts = (struct Pointf*)verts_;
	struct RpPoint v1, v2;
	struct Pointf p1, p2, p3;
	float sinA, sinA90;
	...
	{
		p2 = verts[(i) % count]; // The corner point that is being rounded.
		p3 = verts[(i + 1) % count];

		// Get the corner as vectors out away from corner.
		v1 = ToRpPoint(p2.x, p2.y, p1.x, p1.y); // vec back from corner point.
		v2 = ToRpPoint(p2.x, p2.y, p3.x, p3.y); // vec forward from corner point.
		...
		angle = nvg__asinf(sinA);
		radDirection = 1; // May need to reverse the radius.
		drawDirection = 0; // May need to draw the arc anticlockwise.
		...
		if (sinA90 < 0)
		{
			if (angle < 0)
			{
				angle = NVG_PI + angle; // Add 180 to move us to the 3 quadrant.
			}
			else
			{
				angle = NVG_PI - angle; // Move back into the 2nd quadrant.
				radDirection = -1;
				drawDirection = 1;
			}
		}
		...
		lenOut = nvg__absf(nvg__cosf(halfAngle) * r / nvg__sinf(halfAngle));
		float minLen = v1.len < v2.len ? v1.len : v2.len;
		if (lenOut > minLen / 2)
		{
			// fix if longer than half line length
			lenOut = minLen;
			// adjust the radius of corner rounding to fit
			cRadius = nvg__absf(lenOut * nvg__sinf(halfAngle) / nvg__cosf(halfAngle));
		}
		else { cRadius = r; }

		float x, y; // x,y is the rounded corner circle center
		...
		x += -v2.ny * cRadius * (float)radDirection;
		y += v2.nx * cRadius * (float)radDirection;

		// draw the arc clockwise
		nvgArc(
		    ctx, x, y, cRadius, (float)(v1.ang + NVG_PI / 2.f * (float)radDirection),
		    (float)(v2.ang - NVG_PI / 2 * (float)radDirection),
		    drawDirection ? NVG_CCW : NVG_CW);
    ...
    float* bounds)
{
	...
	if (bounds != NULL) fonsLineBounds(ctx->fs, y, &bounds[1], &bounds[3]);
	return width;
}
