...
ie::gjk::RayCastCircle(const RayCast* input, const Circle* shape)
{
	...
	vec2 s = SubV(input->p1, p);
	float length;
	vec2 d = NormV(SubV(input->p2, input->p1), length);
	if (length == 0.0f)
	{
		// zero length ray
		return output;
	...
	vec2 hitPoint = MulAddV(s, fraction, d);
...
ie::gjk::RayCastCapsule(const RayCast* input, const Capsule* shape)
{
	...
	float rayLength;
	vec2 u = NormV(SubV(p2, p1), rayLength);
	...
	vec2 b1 = MulSubV(q, radius, n);
	vec2 b2 = MulAddV(q, radius, n);
