...
#ifndef NDEBUG
int
_checkGLError(const char* call, const char* file, int line)
{
	GLenum err;
...
#	define GL_CHECK(stmt)                                                                    \
		do {                                                                                  \
			stmt;                                                                             \
			g_hasGLError = checkGLError(#stmt) || g_hasGLError;                               \
		} while (0)
#else
...
static void
fillRect(NVGcontext* vg, float x, float y, float w, float h, NVGcolor color)
{
...
gammaTests(NVGcontext* vg)
{
	...
	if (img < 0)
	{
		...
		imgpaint = nvgImagePattern(vg, 0, 0, (float)imgw, (float)imgh, 0, img, 1.0f);
	}

	// 50/50 B/W dither vs. #7F7F7F rectangle
	nvgBeginPath(vg);
	nvgRect(vg, 0.f, 0.f, (float)imgw, (float)imgh);
...
static void
gammaTests2(NVGcontext* vg, int testNum)
{
	...
	{
		nvgBeginPath(vg);
		nvgMoveTo(vg, 100, 400.f + 2.f * (float)ii);
		nvgLineTo(vg, 700, 400.f + 20.f * (float)ii);
		nvgStroke(vg);
	}
	//nvgStroke(vg);
...
static void
convexFillTest(NVGcontext* vg)
{
	nvgShapeAntiAlias(vg, 0);

	fillRect(vg, 100, 100, 200, 2, nvgRGBA(0, 0, 0, 255));
	fillRect(vg, 100, 200, 200, 1, nvgRGBA(0, 0, 0, 255));
	fillRect(vg, 100, 300, 200, 0.75f, nvgRGBA(0, 0, 0, 255));
	fillRect(vg, 100, 400, 200, 0.51f, nvgRGBA(0, 0, 0, 255));
	fillRect(vg, 100, 500, 200, 0.5f, nvgRGBA(0, 0, 0, 255));
	fillRect(vg, 100, 500, 200, 0.49f, nvgRGBA(0, 0, 0, 255));
	...
	nvgRect(vg, 350, 300, 200, 0.75f);
	nvgRect(vg, 350, 400, 200, 0.51f);
	nvgRect(vg, 350, 500, 200, 0.5f);
	nvgRect(vg, 350, 500, 200, 0.49f);
	nvgFill(vg);
}
...
static NVGpaint
linearGradToNVGpaint(NVGcontext* vg, NSVGgradient* g)
{
	...
	return nvgLinearGradient(
	    vg, x0 + s0->offset * dx, y0 + s0->offset * dy, x0 + s1->offset * dx,
	    y0 + s1->offset * dy, nvgColUint(s0->color), nvgColUint(s1->color));
}

static void
nvgTransformArray(struct NVGcontext* vg, float* t)
{
	nvgSetTransform(vg, t[0], t[1], t[2], t[3], t[4], t[5]);
}

static void
nsvgRender(NVGcontext* vg, NSVGimage* image, float bounds[4])
{
	...
	nvgSetTransform(vg, s, 0, 0, s, bounds[0], bounds[1]);
		...
		if (shape->paths)
		{
			...
			for (NSVGpath* path = shape->paths; path != NULL; path = path->next)
			{
				...
				for (int i = 1; i < path->npts - 1; i += 3)
				{
					...
					if (fabsf((p[2] - p[0]) * (p[5] - p[1]) - (p[4] - p[0]) * (p[3] - p[1])) <
					    0.001f)
						nvgLineTo(vg, p[4], p[5]);
					else nvgBezierTo(vg, p[0], p[1], p[2], p[3], p[4], p[5]);
				}
			...
			if (shape->fill.type != NSVG_PAINT_NONE)
			{
				if (shape->fill.type == NSVG_PAINT_COLOR)
					nvgFillColor(
					    vg, nvgColUint(shape->fill.color)); // fill.color includes opacity
...
static void
svgTest(NVGcontext* vg, const char* filename, int fbWidth, int fbHeight)
{
...
static void
smallPathsTest(NVGcontext* vg, int fbWidth, int fbHeight)
{
	//uint64_t t0 = SDL_GetPerformanceCounter();
	nvgFillColor(vg, nvgRGBA(0, 0, 255, 128));
	for (int y = 0; y < fbHeight; y += 20)
	{ // was fbHeight
		for (int x = 0; x < fbWidth; x += 20)
		{ // was fbWidth
			nvgBeginPath(vg);
			...
			{
				nvgLineTo(vg, (float)x + 4 * (float)(dy % 2), (float)(y + dy));
			}
			nvgLineTo(vg, (float)x + 8, (float)y + 50);
			nvgLineTo(vg, (float)x + 8, (float)y);
			nvgClosePath(vg);
			nvgFill(vg);
		}
	}
...
static void
bigPathsTest(NVGcontext* vg, int xpaths, int ypaths, int fbWidth, int fbHeight)
{
...
static int
freadall(const char* filename, char** bufferout)
{
...
static void
textPerformance(NVGcontext* vg, int testNum, float fontsize, float fontblur)
{
	...
	char longstr[96];
	for (char c = 32; c < 127; ++c) longstr[(int)c - 32] = c;
	longstr[95] = '\0';

	for (int y = 20; y < 2200; y += fontsize - 4) nvgText(vg, 250, y, longstr, NULL);
	// reset blur
	nvgFontBlur(vg, 0);
}

static void
textTests(NVGcontext* vg)
{
	/*
...
static void
textTest2(NVGcontext* vg, int testNum)
{
	int asPaths = testNum % 2;
	nvgAtlasTextThreshold(vg, asPaths ? 0.0f : 48.0f);
	nvgFontFace(vg, "sans");
...
linearGradTest(NVGcontext* vg)
{
	...
	nvgFillPaint(
	    vg,
	    nvgLinearGradient(vg, 0, 0, 1, 0, nvgRGBA(255, 0, 0, 255), nvgRGBA(0, 0, 255, 255)));
	nvgResetTransform(vg);
	nvgSetTransform(vg, m[0], m[1], m[2], m[3], m[4], m[5]);
	nvgBeginPath(vg);
...
static void
imageTest(NVGcontext* vg, int imgHandle, float x, float y, float w, float h)
{
	int imgw, imgh;
