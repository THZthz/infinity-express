...
inline void
throw_exception_assert_compatibility(std::exception const &)
{
}
...
operator==(const default_allocator<T> &, const default_allocator<U> &) noexcept
{
	return true;
}
...
template <class A, class = void> struct alloc_size_type
{
	typedef typename std::make_unsigned<
	    typename boost::allocator_difference_type<A>::type>::type type;
};

template <class A> struct alloc_size_type<A, typename alloc_void<typename A::size_type>::type>
{
...
{
	template <class O>
	static auto check(int) -> alloc_no<decltype(std::declval<O &>().construct(
	    std::declval<T *>(),
	    std::declval<Args &&>()...))>;
...
template <class A, class T> class alloc_has_destroy
{
	template <class O>
	static auto
	check(int) -> alloc_no<decltype(std::declval<O &>().destroy(std::declval<T *>()))>;
...
template <class A>
inline typename detail::
    alloc_if<detail::alloc_has_max_size<A>::value, typename allocator_size_type<A>::type>::type
...
template <class A>
inline typename detail::alloc_if<
...
{
	template <class O>
	static auto check(int) -> alloc_no<
	    decltype(std::declval<const O &>().select_on_container_copy_construction())>;
...
template <class T, class U>
inline bool
operator==(const noinit_adaptor<T> &lhs, const noinit_adaptor<U> &rhs) noexcept
{
...
noinit_adapt(const A &a) noexcept
{
...
template <class T>
inline void
checked_array_delete(T *x) noexcept
{
	static_assert(sizeof(T) != 0, "Type must be complete");
	delete[] x;
}
...
#	undef HAS_IA32_PAUSE
    ...
    ((defined(__ARM_ARCH) && __ARM_ARCH >= 8) || defined(__ARM_ARCH_8A__) ||                  \
     defined(__aarch64__))
...
inline void
lw_thread_join(lw_thread_t thread)
{
...
template <class F>
int
lw_thread_create(lw_thread_t &th, F f)
{
...
struct is_pod
    : public integral_constant<
          bool,
          ::boost::is_scalar<T>::value || ::boost::is_void<T>::value || std::is_pod<T>::value>
{
};
...
template <std::size_t Target> struct double_alignment<Target, false>
{
	typedef typename long_double_alignment<
	    Target,
	    boost::alignment_of<long double>::value >= Target>::type type;
};
...
{
	typedef typename double_alignment<Target, boost::alignment_of<double>::value >= Target>::
	    type type;
};
...
{
	typedef
	    typename int_alignment<Target, boost::alignment_of<int>::value >= Target>::type type;
};
...
{
	typedef typename short_alignment<Target, boost::alignment_of<short>::value >= Target>::type
	    type;
};
...
template <std::size_t Align> struct type_with_alignment
{
	typedef typename boost::detail::
	    char_alignment<Align, boost::alignment_of<char>::value >= Align>::type type;
};
...
template <unsigned size, unsigned align_> struct allocator_impl
{
	...
	{
		static freeblock<
		    sizeof(lightweight_mutex), boost::alignment_of<lightweight_mutex>::value>
		    fbm;
...
lightweight_mutex
    *allocator_impl<size, align_>::mutex_init = &allocator_impl<size, align_>::mutex();
...
yield(unsigned k)
{
	...
	if (k & 1) { boost::core::sp_thread_sleep(); }
	else { boost::core::sp_thread_pause(); }
}
...
inline void
atomic_increment(std::atomic_int_least32_t *pw) noexcept
{
	pw->fetch_add(1, std::memory_order_relaxed);
}

inline std::int_least32_t
atomic_decrement(std::atomic_int_least32_t *pw) noexcept
{
...
template <class D>
D *
get_local_deleter(D * /*p*/) noexcept
{
	return 0;
}
...
template <class T>
inline bool
operator==(scoped_ptr<T> const &p, std::nullptr_t) noexcept
{
	return p.get() == 0;
}
...
template <class T>
inline bool
operator==(std::nullptr_t, scoped_ptr<T> const &p) noexcept
{
	return p.get() == 0;
}
...
template <class T>
inline void
swap(scoped_ptr<T> &a, scoped_ptr<T> &b) noexcept
{
	a.swap(b);
}
...
template <class T>
inline T *
get_pointer(scoped_ptr<T> const &p) noexcept
{
	return p.get();
}
...
template <class T>
inline bool
operator==(scoped_array<T> const &p, std::nullptr_t) noexcept
{
	return p.get() == 0;
}
...
template <class T>
inline bool
operator==(std::nullptr_t, scoped_array<T> const &p) noexcept
{
	return p.get() == 0;
}
...
template <class T>
inline void
swap(scoped_array<T> &a, scoped_array<T> &b) noexcept
{
	a.swap(b);
}
...
public:
	...
	template <class P, class D, class A> shared_count(P p, D d, A a) : pi_(0)
	{
		...
		if (pi_ != 0) { ::new (static_cast<void *>(pi_)) impl_type(p, d, a); }
		else
		{
			d(p);
			boost::throw_exception(std::bad_alloc());
	...
	template <class P, class D, class A> shared_count(P p, sp_inplace_tag<D>, A a) : pi_(0)
	{
		...
		if (pi_ != 0) { ::new (static_cast<void *>(pi_)) impl_type(p, a); }
		else
		{
			D::operator_fn(p);
			boost::throw_exception(std::bad_alloc());
...
inline bool
shared_count::operator==(weak_count const &r) const noexcept
{
	return pi_ == r.pi_;
}
...
template <class Y, class T>
inline void
sp_assert_convertible() noexcept
{
...
public:
	...
	operator[](std::ptrdiff_t i) const noexcept
	{
		...
		assert(
		    i >= 0 &&
		    (i < boost::detail::sp_extent<T>::value ||
...
template <class T>
inline bool
operator!=(shared_ptr<T> const &a, shared_ptr<T> const &b) noexcept
{
	return a.get() != b.get();
}
...
template <class T>
inline bool
operator==(shared_ptr<T> const &p, std::nullptr_t) noexcept
{
	return p.get() == 0;
}
...
template <class T>
inline bool
operator==(std::nullptr_t, shared_ptr<T> const &p) noexcept
{
	return p.get() == 0;
}
...
template <class T, class U>
shared_ptr<T>
static_pointer_cast(shared_ptr<U> const &r) noexcept
{
...
template <class T, class U>
shared_ptr<T>
const_pointer_cast(shared_ptr<U> const &r) noexcept
{
...
template <class T, class U>
shared_ptr<T>
dynamic_pointer_cast(shared_ptr<U> const &r) noexcept
{
...
template <class T, class U>
shared_ptr<T>
reinterpret_pointer_cast(shared_ptr<U> const &r) noexcept
{
...
template <class T, class U>
shared_ptr<T>
dynamic_pointer_cast(shared_ptr<U> &&r) noexcept
{
	(void)dynamic_cast<T *>(static_cast<U *>(0));
...
D *
basic_get_deleter(shared_ptr<T> const &p) noexcept
{
...
template <class T>
inline bool
atomic_is_lock_free(shared_ptr<T> const * /*p*/) noexcept
{
	return false;
}
...
template <class T>
shared_ptr<T>
atomic_load(shared_ptr<T> const *p) noexcept
{
...
template <class T>
void
atomic_store(shared_ptr<T> *p, shared_ptr<T> r) noexcept
{
	boost::detail::spinlock_pool<2>::scoped_lock lock(p);
	p->swap(r);
}
...
template <class T>
shared_ptr<T>
atomic_exchange(shared_ptr<T> *p, shared_ptr<T> r) noexcept
{
...
template <class T>
bool
atomic_compare_exchange(shared_ptr<T> *p, shared_ptr<T> *v, shared_ptr<T> w) noexcept
{
	boost::detail::spinlock &sp = boost::detail::spinlock_pool<2>::spinlock_for(p);
...
template <class T>
std::size_t
hash_value(boost::shared_ptr<T> const &p) noexcept
{
...
template <class D>
D *
get_local_deleter(local_sp_deleter<D> *p) noexcept
{
	return &p->deleter();
}

inline void *
get_local_deleter(local_sp_deleter<void> * /*p*/) noexcept
{
	return 0;
}
...
template <class D, class T>
D *
basic_get_local_deleter(D *, shared_ptr<T> const &p) noexcept
{
...
template <class D, class T>
D const *
basic_get_local_deleter(D const *, shared_ptr<T> const &p) noexcept
{
	return static_cast<D *>(p._internal_get_local_deleter(typeid(local_sp_deleter<D>)));
}
...
inline void
lsp_pointer_construct(
    boost::local_shared_ptr<E[N]> * /*ppx*/,
    Y *p,
    boost::detail::local_counted_base *&pn)
{
...
public:
	...
	operator shared_ptr<Y>() const noexcept
	{
		...
		{
			return shared_ptr<Y>(
			    boost::detail::sp_internal_constructor_tag(), px,
			    pn->local_cb_get_shared_count());
		}
		else { return shared_ptr<Y>(); }
	}
...
template <class T>
inline bool
operator==(local_shared_ptr<T> const &p, std::nullptr_t) noexcept
{
	return p.get() == 0;
}
...
template <class T>
inline bool
operator==(std::nullptr_t, local_shared_ptr<T> const &p) noexcept
{
	return p.get() == 0;
}
...
template <class T>
inline void
swap(local_shared_ptr<T> &a, local_shared_ptr<T> &b) noexcept
{
	a.swap(b);
}
...
template <class T, class U>
local_shared_ptr<T>
static_pointer_cast(local_shared_ptr<U> const &r) noexcept
{
	(void)static_cast<T *>(static_cast<U *>(0));
...
template <class T, class U>
local_shared_ptr<T>
const_pointer_cast(local_shared_ptr<U> const &r) noexcept
{
	(void)const_cast<T *>(static_cast<U *>(0));
...
template <class T, class U>
local_shared_ptr<T>
dynamic_pointer_cast(local_shared_ptr<U> const &r) noexcept
{
	(void)dynamic_cast<T *>(static_cast<U *>(0));
...
template <class T, class U>
local_shared_ptr<T>
reinterpret_pointer_cast(local_shared_ptr<U> const &r) noexcept
{
	(void)reinterpret_cast<T *>(static_cast<U *>(0));
...
template <class T, class U>
local_shared_ptr<T>
static_pointer_cast(local_shared_ptr<U> &&r) noexcept
{
	(void)static_cast<T *>(static_cast<U *>(0));
...
template <class T, class U>
local_shared_ptr<T>
const_pointer_cast(local_shared_ptr<U> &&r) noexcept
{
	(void)const_cast<T *>(static_cast<U *>(0));
...
template <class T, class U>
local_shared_ptr<T>
dynamic_pointer_cast(local_shared_ptr<U> &&r) noexcept
{
	(void)dynamic_cast<T *>(static_cast<U *>(0));
...
template <class T, class U>
local_shared_ptr<T>
reinterpret_pointer_cast(local_shared_ptr<U> &&r) noexcept
{
	(void)reinterpret_cast<T *>(static_cast<U *>(0));
...
template <class D, class T>
D *
get_deleter(local_shared_ptr<T> const &p) noexcept
{
...
template <class T>
std::size_t
hash_value(local_shared_ptr<T> const &p) noexcept
{
...
template <class T>
inline bool
operator==(shared_array<T> const &a, shared_array<T> const &b) noexcept
{
	return a.get() == b.get();
}
...
template <class T>
inline bool
operator==(shared_array<T> const &p, std::nullptr_t) noexcept
{
	return p.get() == 0;
}
...
template <class T>
inline bool
operator==(std::nullptr_t, shared_array<T> const &p) noexcept
{
	return p.get() == 0;
}
...
template <class T>
void
swap(shared_array<T> &a, shared_array<T> &b) noexcept
{
	a.swap(b);
}
...
template <class D, class T>
D *
get_deleter(shared_array<T> const &p) noexcept
{
	return static_cast<D *>(p._internal_get_deleter(typeid(D)));
}
...
template <class T>
std::size_t
hash_value(boost::intrusive_ptr<T> const &p) noexcept
{
...
template <class T>
std::size_t
hash_value(boost::weak_ptr<T> const &p) noexcept
{
...
{
	bool
	operator()(::boost::weak_ptr<T> const &a, ::boost::weak_ptr<T> const &b) const noexcept
	{
...
{
	return shared_ptr<T>(
	    p->enable_shared_from_this<enable_shared_from>::shared_from_this(), p);
}

template <class T>
weak_ptr<T>
weak_from(T *p) noexcept
{
...
protected:
	...
	{
		assert(
		    shared_this_.use_count() <= 1); // make sure no dangling shared_ptr objects exist
	}
	...
	template <class X, class Y> void _internal_accept_owner(shared_ptr<X> *ppx, Y *) const
	{
		...
		else if (shared_this_.use_count() != 0)
		{
			...
			detail::esft2_deleter_wrapper
			    *pd = boost::get_deleter<detail::esft2_deleter_wrapper>(shared_this_);
			assert(pd != 0);
...
constexpr inline typename detail::make_scalar<T>::type *
...
template <class T, class U> struct sp_array_alignment
{
	...
	{
		value =
		    sp_max_size<boost::alignment_of<T>::value, boost::alignment_of<U>::value>::value
	};
};
...
public:
	...
	sp_array_base(const A &other, type *start, std::size_t size) : state_(other, size)
	{
		boost::alloc_construct_n(
		    state_.allocator(), boost::first_scalar(start),
	    ...
	    : state_(other, size)
	{
		...
		boost::alloc_construct_n(
		    state_.allocator(), boost::first_scalar(start), state_.size() * count,
	...
	void dispose() noexcept override
	{
		boost::alloc_destroy_n(
		    state_.allocator(), boost::first_scalar(sp_array_start<type>(this)),
		    state_.size() * sp_array_count<type>::value);
	}
...
public:
	template <class U>
	lsp_array_state(const U &other, std::size_t size) noexcept : sp_array_state<A>(other, size)
	{
	}
...
template <class A, std::size_t N> class lsp_size_array_state : public sp_size_array_state<A, N>
{
public:
	template <class U>
...
make_shared_noinit()
{
	boost::shared_ptr<T> pt(
	    static_cast<T *>(0), boost::detail::sp_inplace_tag<boost::detail::sp_ms_deleter<T>>());
...
allocate_local_shared(A const &a, Args &&...args)
{
	...
	std::allocator_traits<A2>::construct(
	    a2, static_cast<T *>(pv), std::forward<Args>(args)...);
...
template <class T, class U>
inline T *
static_pointer_cast(U *ptr) noexcept
{
...
template <class T, class U>
inline T *
dynamic_pointer_cast(U *ptr) noexcept
{
...
template <class T, class U>
inline T *
const_pointer_cast(U *ptr) noexcept
{
...
template <class T, class U>
inline T *
reinterpret_pointer_cast(U *ptr) noexcept
{
...
template <class T, class U>
std::unique_ptr<T>
static_pointer_cast(std::unique_ptr<U> &&r) noexcept
{
	(void)static_cast<T *>(static_cast<U *>(0));
...
template <class T, class U>
std::unique_ptr<T>
dynamic_pointer_cast(std::unique_ptr<U> &&r) noexcept
{
	(void)dynamic_cast<T *>(static_cast<U *>(0));
...
template <class T, class U>
std::unique_ptr<T>
const_pointer_cast(std::unique_ptr<U> &&r) noexcept
{
	(void)const_cast<T *>(static_cast<U *>(0));
...
template <class T, class U>
std::unique_ptr<T>
reinterpret_pointer_cast(std::unique_ptr<U> &&r) noexcept
{
	(void)reinterpret_cast<T *>(static_cast<U *>(0));
...
template <class T>
T *
get_pointer(std::auto_ptr<T> const &p)
{
	return p.get();
}
...
template <class T>
T *
get_pointer(std::unique_ptr<T> const &p)
{
	return p.get();
}
...
template <class T>
T *
get_pointer(std::shared_ptr<T> const &p)
{
	return p.get();
}
...
template <class T, class P>
inline bool
operator==(const sp_alloc_ptr<T, P> &lhs, std::nullptr_t) noexcept
{
	return !lhs.ptr();
}
...
template <class T, class P>
inline bool
operator==(std::nullptr_t, const sp_alloc_ptr<T, P> &rhs) noexcept
{
	return !rhs.ptr();
}
...
#endif
	boost::alloc_destroy_n(
	    a, boost::first_scalar(boost::to_address(p)),
...
template <class T, class A>
class alloc_deleter
    : empty_value<typename allocator_rebind<A, typename detail::sp_alloc_value<T>::type>::type>
{
	typedef
...
inline typename enable_if_<
    is_bounded_array<T>::value,
    std::unique_ptr<
        typename detail::sp_alloc_result<T>::type,
...
template <class U, class T> struct def_del_compatible_cond : bmupmu::is_convertible<U *, T *>
{
};
...
template <class T>
void
call_delete(T *p, is_array_del<true>)
{
	delete[] p;
}
...
template <class T>
void
call_delete(T *p, is_array_del<false>)
{
	delete p;
}
...
template <
    ...
    bool enable =
        ...
        !move_upmu::is_same<
            typename move_upmu::remove_cv<U>::type,
...
template <class T> struct get_cvelement : bmupmu::remove_cv<typename get_element_type<T>::type>
{
};
...
template <class P1, class P2> struct is_same_cvelement_and_convertible
{
	...
	static const bool same_cvless = bmupmu::
...
struct is_unique_ptr_convertible : is_same_cvelement_and_convertible<FromPointer, ThisPointer>
{
};
...
struct enable_up_ptr
    : bmupmu::enable_if_c<
          is_unique_ptr_convertible<bmupmu::is_array<T>::value, FromPointer, ThisPointer>::
              value,
          Type>
{
};
...
public:
	    ...
	    : m_data(p)
	{
		...
		static_assert(
		    (!bmupd::missing_virtual_destructor<
...
template <class T, class D>
FORCEINLINE bool
operator==(const unique_ptr<T, D> &x, bmupd::nullptr_type) noexcept
{
	return !x;
}
...
template <class T, class D>
FORCEINLINE bool
operator==(bmupd::nullptr_type, const unique_ptr<T, D> &x) noexcept
{
	return !x;
}
...
template <class T, class D>
FORCEINLINE bool
operator<(const unique_ptr<T, D> &x, bmupd::nullptr_type)
{
...
template <class T, class D>
FORCEINLINE bool
operator<(bmupd::nullptr_type, const unique_ptr<T, D> &x)
{
