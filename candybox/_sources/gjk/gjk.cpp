...
ie::gjk::SegmentDistance(glm::vec2 p1, glm::vec2 q1, glm::vec2 p2, glm::vec2 q2)
{
	SegmentDistanceResult result{};

	glm::vec2 d1 = SubV(q1, p1);
	glm::vec2 d2 = SubV(q2, p2);
	glm::vec2 r = SubV(p1, p2);
...
gjk::DistanceProxy
ie::gjk::MakeProxy(const glm::vec2* vertices, int32_t count, float radius)
{
...
ie::gjk::ShapeDistance(DistanceCache* RESTRICT cache, const DistanceInput* RESTRICT input)
{
	...
	if (input->useRadii)
	{
		...
		else
		{
			...
			glm::vec2 normal = NormV(SubV(output.pointB, output.pointA));
...
FindMinSeparation(const SepFunc* f, int32_t* indexA, int32_t* indexB, float t)
{
	...
	switch (f->type)
	{
		case SepType::kPoint: {
			...
			glm::vec2 axisB = InvRotateVector(xfB.q, NegV(f->axis));
		...
		case SepType::kFaceA: {
			...
			glm::vec2 axisB = InvRotateVector(xfB.q, NegV(normal));
		...
		case SepType::kFaceB: {
			...
			glm::vec2 axisA = InvRotateVector(xfA.q, NegV(normal));
...
bool
ie::gjk::PointInCircle(glm::vec2 point, const Circle* shape)
{
...
bool
ie::gjk::PointInCapsule(glm::vec2 point, const Capsule* shape)
{
...
bool
ie::gjk::PointInPolygon(glm::vec2 point, const Polygon* shape)
{
	DistanceInput input{};
...
ie::gjk::ComputeCapsuleAABB(const Capsule* shape, const Xf2d& xf)
{
	...
	glm::vec2 lower = SubV(MinV(v1, v2), r);
	glm::vec2 upper = AddV(MaxV(v1, v2), r);
...
ie::gjk::ComputeSegmentAABB(const Segment* shape, const Xf2d& xf)
{
	...
	glm::vec2 lower = MinV(v1, v2);
	glm::vec2 upper = MaxV(v1, v2);
...
RecurseHull(glm::vec2 p1, glm::vec2 p2, glm::vec2* ps, int32_t count)
{
	...
	glm::vec2 e = NormV(SubV(p2, p1));

	// discard points left of e and find point furthest to the right of e
	glm::vec2 rightPoints[ie::gjk::MAX_POLY_VERTS];
...
gjk::Hull
ie::gjk::ComputeHull(const glm::vec2* points, int32_t count)
{
	Hull hull;
	hull.count = 0;
...
ie::gjk::ValidateHull(const Hull* hull)
{
	...
	for (int32_t i = 0; i < hull->count; ++i)
	{
		...
		glm::vec2 e = NormV(SubV(hull->points[i2], p));
