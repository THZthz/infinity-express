...
ie::gjk::MakePolygon(const Hull *hull, float radius)
{
	...
	for (int32_t i = 0; i < shape.count; ++i)
	{
		...
		glm::vec2 edge = SubV(shape.vertices[i2], shape.vertices[i1]);
...
gjk::Polygon
ie::gjk::MakePolygon(const glm::vec2 *vertices, int count, float radius)
{
...
gjk::Polygon
ie::gjk::MakeOffsetBox(float hx, float hy, glm::vec2 center, float angle)
{
	Xf2d xf;
	xf.p = center;
	xf.q.set(angle);
...
gjk::Polygon
ie::gjk::MakeCapsule(glm::vec2 p1, glm::vec2 p2, float radius)
{
	Polygon shape;
	shape.vertices[0] = p1;
	shape.vertices[1] = p2;

	glm::vec2 axis = NormV(SubV(p2, p1));
...
ShapeProject(const glm::vec2 *ta, uint32_t na, const glm::vec2 *tb, uint32_t nb)
{
	for (unsigned i = 0; i < na; i++)
	{
		...
		glm::vec2 edgePerpendicular = CCW90V(edge);
    ...
    glm::vec2 &out)
{
	...
	glm::vec2 originDist = SubV(b.p1, a.p1);
