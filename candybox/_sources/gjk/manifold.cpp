...
ie::gjk::GJK_Circles(const Circle* A, Xf2d xfA, const Circle* B, Xf2d xfB, float maxd)
{
	...
	float distance;
	glm::vec2 normal = NormV(SubV(pointB, pointA), distance);
	...
	glm::vec2 cA = MulAddV(pointA, rA, normal);
	glm::vec2 cB = MulAddV(pointB, -rB, normal);
...
gjk::Manifold
ie::gjk::GJK_Capsule2Circle(const Capsule* A, Xf2d xfA, const Circle* B, Xf2d xfB, float maxd)
{
	Manifold manifold{};
	...
	float distance;
	glm::vec2 normal = NormV(SubV(pB, pA), distance);
	...
	glm::vec2 cA = MulAddV(pA, rA, normal);
	glm::vec2 cB = MulAddV(pB, -rB, normal);
...
gjk::Manifold
ie::gjk::GJK_Poly2Circle(const Polygon* A, Xf2d xfA, const Circle* B, Xf2d xfB, float maxd)
{
	Manifold manifold{};
	...
	if (u1 < 0.0f && separation > FLT_EPSILON)
	{
		...
		glm::vec2 normal = NormV(SubV(c, v1));
		manifold.pointCount = 1;
		manifold.normal = RotateVector(xfA.q, normal);
		glm::vec2 cA = v1;
		glm::vec2 cB = MulAddV(c, -radius, normal);
		manifold.points[0].point = TransformPoint(xfA, LerpV(cA, cB, 0.5f));
	...
	else if (u2 < 0.0f && separation > FLT_EPSILON)
	{
		...
		glm::vec2 normal = NormV(SubV(c, v2));
		manifold.pointCount = 1;
		manifold.normal = RotateVector(xfA.q, normal);
		glm::vec2 cA = v2;
		glm::vec2 cB = MulAddV(c, -radius, normal);
	...
	else
	{
		...
		glm::vec2 cA = MulAddV(c, -DotV(SubV(c, v1), normal), normal);
    ...
    bool flip)
{
	...
	glm::vec2 tangent = CrossSV(1.0f, normal);
    ...
    gjk::DistanceCache* cache)
{
	...
	if (a1 == a2)
	{
		...
		glm::vec2 axis = InvRotateVector(xfB.q, SubV(output.pointA, output.pointB));
	...
	else
	{
		// Find reference edge that most aligns with vector between closest points.
		// This works for capsules and polygons
		glm::vec2 axis = InvRotateVector(xfA.q, SubV(output.pointB, output.pointA));
