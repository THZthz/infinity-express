...
static IE_THREAD_LOCAL uint32_t gtl_threadNum = ie::NO_THREAD_NUM;
...
class TaskPipe : public LockLessMultiReadPipe<gc_PipeSizeLog2, ie::SubTaskSet>
{
};
...
enum ThreadState : int32_t
{
	...
	IE_THREAD_STATE_NOT_LAUNCHED, // for debug purposes - indicates ie task thread not yet launched
...
struct alignas(ie::gc_CacheLineSize) ThreadDataStore
{
	...
	char prevent_false_Share
	    [ie::gc_CacheLineSize - sizeof(std::atomic<ThreadState>) - sizeof(semaphoreid_t*) -
...
static_assert(
    SIZEOFTHREADDATASTORE == ie::gc_CacheLineSize,
...
IE_TS_API void
ie::DefaultFreeFunc(void* ptr_, size_t size_, void* userData_, const char* file_, int line_)
{
	(void)size_;
	(void)userData_;
	(void)file_;
...
{
	assert(gtl_threadNum != ie::NO_THREAD_NUM);
	...
	if (threadState == IE_THREAD_STATE_EXTERNAL_REGISTERED)
	{
		...
		gtl_threadNum = ie::NO_THREAD_NUM;
	}
}
...
void
TaskScheduler::
    WaitforTask(const ICompletable* pCompletable_, ie::TaskPriority priorityOfLowestToRun_)
{
	assert(gtl_threadNum != ie::NO_THREAD_NUM);
	uint32_t threadNum = gtl_threadNum;
	uint32_t hintPipeToCheck_io = threadNum + 1; // does not need to be clamped.
	...
	while (GetIsRunning() && (bHaveTasks || otherThreadsRunning))
	{
		...
		if (spinCount > gc_SpinCount)
		{
			...
			do {
				...
				// We can only add a pinned task to wait on if we find an ie Task Thread which isn't this thread.
