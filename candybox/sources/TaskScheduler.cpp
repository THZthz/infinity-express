...
#if defined(IE_CUSTOM_ALLOC_FILE_AND_LINE)
#	define IE_FILE_AND_LINE __FILE__, __LINE__
#else
namespace {
const char* gc_File = "";
const uint32_t gc_Line = 0;
} // namespace
#	define IE_FILE_AND_LINE gc_File, gc_Line
#endif
...
#	define IE_USE_WINDOWS_PROCESSOR_API
#endif

#ifdef IE_USE_WINDOWS_PROCESSOR_API
...
uint32_t
candybox::GetNumHardwareThreads()
{
#ifdef IE_USE_WINDOWS_PROCESSOR_API
...
// For XCode before version 8 thread_local is not defined, so add to your compile defines: IE_THREAD_LOCAL __thread
#ifndef IE_THREAD_LOCAL
#	if defined(_MSC_VER) && _MSC_VER <= 1800
#		define IE_THREAD_LOCAL __declspec(thread)
...
//         #define IE_THREAD_LOCAL __thread
#	else
#		define IE_THREAD_LOCAL thread_local
#	endif
#endif


// each software thread gets its own copy of gtl_threadNum, so this is safe to use as a static variable
static IE_THREAD_LOCAL uint32_t gtl_threadNum = candybox::NO_THREAD_NUM;
...
class TaskPipe : public LockLessMultiReadPipe<gc_PipeSizeLog2, candybox::SubTaskSet>
{
};

enum ThreadState : int32_t
{
	IE_THREAD_STATE_NONE, // shouldn't get this value
	IE_THREAD_STATE_NOT_LAUNCHED, // for debug purposes - indicates candybox task thread not yet launched
	IE_THREAD_STATE_RUNNING,
	IE_THREAD_STATE_PRIMARY_REGISTERED, // primary thread is the one enkiTS was initialized on
	IE_THREAD_STATE_EXTERNAL_REGISTERED,
	IE_THREAD_STATE_EXTERNAL_UNREGISTERED,
	IE_THREAD_STATE_WAIT_TASK_COMPLETION,
	IE_THREAD_STATE_WAIT_NEW_TASKS,
	IE_THREAD_STATE_WAIT_NEW_PINNED_TASKS,
	IE_THREAD_STATE_STOPPED,
};
...
struct alignas(candybox::gc_CacheLineSize) ThreadDataStore
{
	semaphoreid_t* pWaitNewPinnedTaskSemaphore = nullptr;
	std::atomic<ThreadState> threadState = {IE_THREAD_STATE_NONE};
	uint32_t rndSeed = 0;
	char prevent_false_Share
	    [candybox::gc_CacheLineSize - sizeof(std::atomic<ThreadState>) - sizeof(semaphoreid_t*) -
...
static_assert(
    SIZEOFTHREADDATASTORE == candybox::gc_CacheLineSize,
...
namespace {
SubTaskSet
SplitTask(SubTaskSet& subTask_, uint32_t rangeToSplit_)
{
...
void
SafeCallback(ProfilerCallbackFunc func_, uint32_t threadnum_)
{
	if (func_ != nullptr) { func_(threadnum_); }
}
} // namespace


IE_TS_API void*
candybox::DefaultAllocFunc(
    size_t align_,
    size_t size_,
    void* userData_,
    const char* file_,
...
IE_TS_API void
candybox::DefaultFreeFunc(void* ptr_, size_t size_, void* userData_, const char* file_, int line_)
{
	(void)size_;
	(void)userData_;
	(void)file_;
...
bool
TaskScheduler::RegisterExternalTaskThread()
{
	bool bRegistered = false;
	while (!bRegistered &&
	       m_NumExternalTaskThreadsRegistered < (int32_t)m_Config.numExternalTaskThreads)
	{
		for (uint32_t thread = GetNumFirstExternalTaskThread();
		     thread < GetNumFirstExternalTaskThread() + m_Config.numExternalTaskThreads;
		     ++thread)
		{
			ThreadState threadStateExpected = IE_THREAD_STATE_EXTERNAL_UNREGISTERED;
			if (m_pThreadDataStore[thread].threadState.compare_exchange_strong(
			        threadStateExpected, IE_THREAD_STATE_EXTERNAL_REGISTERED))
			{
...
bool
TaskScheduler::RegisterExternalTaskThread(uint32_t threadNumToRegister_)
{
	assert(threadNumToRegister_ >= GetNumFirstExternalTaskThread());
	assert(threadNumToRegister_ <
...
void
TaskScheduler::DeRegisterExternalTaskThread()
{
	assert(gtl_threadNum != candybox::NO_THREAD_NUM);
	assert(gtl_threadNum >= GetNumFirstExternalTaskThread());
	ThreadState threadState =
	    m_pThreadDataStore[gtl_threadNum].threadState.load(std::memory_order_acquire);
	assert(threadState == IE_THREAD_STATE_EXTERNAL_REGISTERED);
	if (threadState == IE_THREAD_STATE_EXTERNAL_REGISTERED)
	{
		--m_NumExternalTaskThreadsRegistered;
		m_pThreadDataStore[gtl_threadNum].threadState.store(
		    IE_THREAD_STATE_EXTERNAL_UNREGISTERED, std::memory_order_release);
		gtl_threadNum = candybox::NO_THREAD_NUM;
	}
}

uint32_t
TaskScheduler::GetNumRegisteredExternalTaskThreads()
{
	return m_NumExternalTaskThreadsRegistered;
}

void
TaskScheduler::TaskingThreadFunction(const ThreadArgs& args_)
{
	uint32_t threadNum = args_.threadNum;
	TaskScheduler* pTS = args_.pTaskScheduler;
	gtl_threadNum = threadNum;

	pTS->m_pThreadDataStore[threadNum]
	    .threadState.store(IE_THREAD_STATE_RUNNING, std::memory_order_release);
	...
	while (pTS->GetIsRunning())
	{
		if (!pTS->TryRunTask(threadNum, hintPipeToCheck_io))
		{
			...
			++spinCount;
			if (spinCount > gc_SpinCount) { pTS->WaitForNewTasks(threadNum); }
			else
			{
	...
	pTS->m_pThreadDataStore[threadNum]
	    .threadState.store(IE_THREAD_STATE_STOPPED, std::memory_order_release);
	SafeCallback(pTS->m_Config.profilerCallbacks.threadStop, threadNum);
}


void
TaskScheduler::StartThreads()
{
	if (m_bHaveThreads) { return; }
	...
	{
		m_pPipesPerThread[priority] = NewArray<TaskPipe>(m_NumThreads, IE_FILE_AND_LINE);
		m_pPinnedTaskListPerThread[priority] =
		    NewArray<PinnedTaskList>(m_NumThreads, IE_FILE_AND_LINE);
	}
	...
	m_pThreadDataStore = NewArray<ThreadDataStore>(m_NumThreads, IE_FILE_AND_LINE);
	m_pThreads = NewArray<std::thread>(m_NumThreads, IE_FILE_AND_LINE);
	m_bRunning = true;
	m_bWaitforAllCalled = false;
	m_bShutdownRequested = false;

	// current thread is primary enkiTS thread
	m_pThreadDataStore[0].threadState = IE_THREAD_STATE_PRIMARY_REGISTERED;
	gtl_threadNum = 0;

	for (uint32_t thread = GetNumFirstExternalTaskThread();
	     thread < m_Config.numExternalTaskThreads + GetNumFirstExternalTaskThread(); ++thread)
	{
		m_pThreadDataStore[thread].threadState = IE_THREAD_STATE_EXTERNAL_UNREGISTERED;
	}
	for (uint32_t thread = m_Config.numExternalTaskThreads + GetNumFirstExternalTaskThread();
	     thread < m_NumThreads; ++thread)
	{
		m_pThreadDataStore[thread].threadState = IE_THREAD_STATE_NOT_LAUNCHED;
	}
	        ...
	        m_NumThreads) // no need to distribute if number of threads requested lower than std::thread sees
	{
		...
		if (success)
		{
			...
			uint32_t currLogicalProcess = GetActiveProcessorCount(
			    (WORD)
			        mainProcessorGroup); // we start iteration at end of current process group's threads
			...
			while (currLogicalProcess < m_NumThreads)
			{
				...
				uint64_t GROUPMASK =
				    ...
				    (64 -
				     groupNumLogicalProcessors); // group mask should not have 1's where there are no processors
				for (uint32_t groupLogicalProcess = 0;
				     (groupLogicalProcess < groupNumLogicalProcessors) &&
				     (currLogicalProcess < m_NumThreads);
				     ++groupLogicalProcess, ++currLogicalProcess)
				{
					if (currLogicalProcess >
					    m_Config.numExternalTaskThreads + GetNumFirstExternalTaskThread())
					{
						...
						if (threadAffinity.Group != currGroup)
						{
							...
							success = SetThreadGroupAffinity(
							    thread_handle, &threadAffinity, nullptr);
							assert(success);
							(void)success;
						}
					}
...
void
TaskScheduler::StopThreads(bool bWait_)
{
	...
	if (m_bHaveThreads)
	{
		...
		DeleteArray(m_pThreadDataStore, m_NumThreads, IE_FILE_AND_LINE);
		DeleteArray(m_pThreads, m_NumThreads, IE_FILE_AND_LINE);
		...
		for (int priority = 0; priority < TASK_PRIORITY_NUM; ++priority)
		{
			DeleteArray(m_pPipesPerThread[priority], m_NumThreads, IE_FILE_AND_LINE);
			m_pPipesPerThread[priority] = NULL;
			DeleteArray(m_pPinnedTaskListPerThread[priority], m_NumThreads, IE_FILE_AND_LINE);
...
bool
TaskScheduler::TryRunTask(uint32_t threadNum_, uint32_t& hintPipeToCheck_io_)
{
	for (int priority = 0; priority < TASK_PRIORITY_NUM; ++priority)
	{
		if (TryRunTask(threadNum_, priority, hintPipeToCheck_io_)) { return true; }
	}
	return false;
}

static inline uint32_t
RotateLeft(uint32_t value, int32_t count)
{
...
bool
TaskScheduler::
    TryRunTask(uint32_t threadNum_, uint32_t priority_, uint32_t& hintPipeToCheck_io_)
{
	...
	if (bHaveTask)
	{
		...
		if (subTask.pTask->m_RangeToRun < partitionSize)
		{
			...
			if (threadNum_ != threadToCheck)
			{
				...
				rangeToSplit = std::max(
				    rangeToSplit,
				    (subTask.partition.end - subTask.partition.start) /
			...
			int prevCount =
			    taskToRun.pTask->m_RunningCount.fetch_sub(1, std::memory_order_acq_rel);
		...
		else
		{
			...
			int prevCount =
			    subTask.pTask->m_RunningCount.fetch_sub(1, std::memory_order_acq_rel);
			if (gc_TaskStartCount == prevCount)
			{
...
void
TaskScheduler::TaskComplete(ICompletable* pTask_, bool bWakeThreads_, uint32_t threadNum_)
{
	...
	assert(
	    gc_TaskAlmostCompleteCount == pTask_->m_RunningCount.load(std::memory_order_acquire));
	bool bCallWakeThreads =
	    bWakeThreads_ && pTask_->m_WaitingForTaskCount.load(std::memory_order_acquire);
	...
	if (bCallWakeThreads) { WakeThreadsForTaskCompletion(); }
	...
	while (pDependent)
	{
		...
		assert(prevDeps < dependenciesCount);
...
bool
TaskScheduler::HaveTasks(uint32_t threadNum_)
{
	for (int priority = 0; priority < TASK_PRIORITY_NUM; ++priority)
	{
		for (uint32_t thread = 0; thread < m_NumThreads; ++thread)
		{
			if (!m_pPipesPerThread[priority][thread].IsPipeEmpty()) { return true; }
		}
		if (!m_pPinnedTaskListPerThread[priority][threadNum_].IsListEmpty()) { return true; }
	}
	return false;
}

void
TaskScheduler::WaitForNewTasks(uint32_t threadNum_)
{
	...
	if (WakeSuspendedThreadsWithPinnedTasks(threadNum_)) { return; }
	...
	ThreadState prevThreadState =
	    m_pThreadDataStore[threadNum_].threadState.load(std::memory_order_relaxed);
	m_pThreadDataStore[threadNum_]
	    .threadState.store(IE_THREAD_STATE_WAIT_NEW_TASKS, std::memory_order_seq_cst);
	...
	m_pThreadDataStore[threadNum_]
	    .threadState.store(prevThreadState, std::memory_order_release);
}

void
TaskScheduler::WaitForTaskCompletion(const ICompletable* pCompletable_, uint32_t threadNum_)
{
	// We don't want to suspend this thread if there are task threads
	...
	ThreadState prevThreadState =
	    m_pThreadDataStore[threadNum_].threadState.load(std::memory_order_relaxed);
	m_pThreadDataStore[threadNum_]
	    .threadState.store(IE_THREAD_STATE_WAIT_TASK_COMPLETION, std::memory_order_seq_cst);
...
void
TaskScheduler::WakeThreadsForNewTasks()
{
	int32_t waiting = m_NumThreadsWaitingForNewTasks.load(std::memory_order_relaxed);
	while (
	    waiting > 0 &&
	    !m_NumThreadsWaitingForNewTasks.compare_exchange_weak(
	        waiting, 0, std::memory_order_release, std::memory_order_relaxed))
	{
	}

	if (waiting > 0) { SemaphoreSignal(*m_pNewTaskSemaphore, waiting); }
...
void
TaskScheduler::WakeThreadsForTaskCompletion()
{
	...
	if (waiting > 0) { SemaphoreSignal(*m_pTaskCompleteSemaphore, waiting); }
}

bool
TaskScheduler::WakeSuspendedThreadsWithPinnedTasks(uint32_t threadNum_)
{
	for (uint32_t t = 1; t < m_NumThreads; ++t)
	{
		...
		assert(state != IE_THREAD_STATE_NONE);

		if (state == IE_THREAD_STATE_WAIT_NEW_TASKS ||
		    state == IE_THREAD_STATE_WAIT_TASK_COMPLETION)
		{
...
void
TaskScheduler::
    SplitAndAddTask(uint32_t threadNum_, SubTaskSet subTask_, uint32_t rangeToSplit_)
{
	int32_t numAdded = 0;
	...
	while (subTask_.partition.start != subTask_.partition.end)
	{
		...
		if (!m_pPipesPerThread[subTask_.pTask->m_Priority][threadNum_].WriterTryWriteFront(
		        taskToAdd))
		{
			--numAdded; // we were unable to add the task
			if (numNewTasksSinceNotification > 1) { WakeThreadsForNewTasks(); }
			...
			{
				taskToAdd.partition
				    .end = taskToAdd.partition.start + taskToAdd.pTask->m_RangeToRun;
	...
	assert(countToRemove > 0);
	int prevCount =
	    subTask_.pTask->m_RunningCount.fetch_sub(countToRemove, std::memory_order_acq_rel);
...
TaskSchedulerConfig
TaskScheduler::GetConfig() const
{
	return m_Config;
}

void
TaskScheduler::AddTaskSetToPipeInt(ITaskSet* pTaskSet_, uint32_t threadNum_)
{
	assert(pTaskSet_->m_RunningCount == gc_TaskStartCount);
	ThreadState prevThreadState =
	    m_pThreadDataStore[threadNum_].threadState.load(std::memory_order_relaxed);
	m_pThreadDataStore[threadNum_]
	    .threadState.store(IE_THREAD_STATE_RUNNING, std::memory_order_relaxed);
	std::atomic_thread_fence(std::memory_order_acquire);
	...
	if (gc_TaskStartCount == prevCount) { TaskComplete(pTaskSet_, true, threadNum_); }
...
void
TaskScheduler::AddTaskSetToPipe(ITaskSet* pTaskSet_)
{
...
void
TaskScheduler::AddPinnedTaskInt(IPinnedTask* pTask_)
{
	...
	if (statePinnedTaskThread == IE_THREAD_STATE_WAIT_NEW_PINNED_TASKS)
	{
		SemaphoreSignal(*m_pThreadDataStore[pTask_->threadNum].pWaitNewPinnedTaskSemaphore, 1);
	}
	else { WakeThreadsForNewTasks(); }
}

void
TaskScheduler::AddPinnedTask(IPinnedTask* pTask_)
{
...
void
TaskScheduler::InitDependencies(ICompletable* pCompletable_)
{
	...
	while (pDependent)
	{
		...
		pDependent->pTaskToRunOnCompletion->m_RunningCount
		    .store(gc_TaskStartCount, std::memory_order_relaxed);
		pDependent = pDependent->pNext;
	}
}
...
void
TaskScheduler::RunPinnedTasks(uint32_t threadNum_, uint32_t priority_)
{
...
void
TaskScheduler::
    WaitforTask(const ICompletable* pCompletable_, candybox::TaskPriority priorityOfLowestToRun_)
{
	assert(gtl_threadNum != candybox::NO_THREAD_NUM);
	uint32_t threadNum = gtl_threadNum;
	uint32_t hintPipeToCheck_io = threadNum + 1; // does not need to be clamped.
	...
	else if (nullptr == pCompletable_)
	{
		for (int priority = 0; priority <= priorityOfLowestToRun_; ++priority)
		{
			if (TryRunTask(threadNum, priority, hintPipeToCheck_io)) { break; }
		}
	}
	...
	while (GetIsRunning() && (bHaveTasks || otherThreadsRunning))
	{
		...
		if (spinCount > gc_SpinCount)
		{
			...
			do {
				...
				// We can only add a pinned task to wait on if we find an candybox Task Thread which isn't this thread.
				...
				{
					ThreadState state =
					    m_pThreadDataStore[dummyWaitTask.threadNum]
					        .threadState.load(std::memory_order_acquire);
					if (state == IE_THREAD_STATE_RUNNING ||
					    state == IE_THREAD_STATE_WAIT_TASK_COMPLETION)
					{
			...
			{
				assert(dummyWaitTask.threadNum != ourThreadNum);
		...
		for (uint32_t thread = 0; thread < m_NumThreads && !otherThreadsRunning; ++thread)
		{
			...
			if (thread != ourThreadNum)
			{
				...
				{
					case IE_THREAD_STATE_NONE: assert(false); break;
					case IE_THREAD_STATE_NOT_LAUNCHED:
					case IE_THREAD_STATE_RUNNING:
					case IE_THREAD_STATE_WAIT_TASK_COMPLETION:
						otherThreadsRunning = true;
						break;
					case IE_THREAD_STATE_WAIT_NEW_PINNED_TASKS:
						otherThreadsRunning = true;
						SemaphoreSignal(
						    *m_pThreadDataStore[thread].pWaitNewPinnedTaskSemaphore, 1);
						break;
					case IE_THREAD_STATE_PRIMARY_REGISTERED:
					case IE_THREAD_STATE_EXTERNAL_REGISTERED:
					case IE_THREAD_STATE_EXTERNAL_UNREGISTERED:
					case IE_THREAD_STATE_WAIT_NEW_TASKS:
					case IE_THREAD_STATE_STOPPED: break;
				}
			}
		}
		if (!otherThreadsRunning)
		{
			...
			{
				// ignore our thread
				if (thread != ourThreadNum) { otherThreadsRunning = HaveTasks(thread); }
			}
		}
	}
...
TaskScheduler::ShutdownNow()
{
	m_bWaitforAllCalled.store(true, std::memory_order_release);
	m_bShutdownRequested.store(true, std::memory_order_release);
	if (m_bHaveThreads) { StopThreads(true); }
}
...
TaskScheduler::WaitForNewPinnedTasks()
{
	...
	ThreadState prevThreadState =
	    m_pThreadDataStore[threadNum].threadState.load(std::memory_order_relaxed);
	m_pThreadDataStore[threadNum]
	    .threadState.store(IE_THREAD_STATE_WAIT_NEW_PINNED_TASKS, std::memory_order_seq_cst);
...
uint32_t
TaskScheduler::GetNumTaskThreads() const
{
	return m_NumThreads;
}
...
uint32_t
TaskScheduler::GetThreadNum() const
{
...
template <typename T>
T*
TaskScheduler::NewArray(size_t num_, const char* file_, int line_)
{
	T* pRet = (T*)m_Config.customAllocator.alloc(
	    alignof(T), num_ * sizeof(T), m_Config.customAllocator.userData, file_, line_);
...
template <typename T>
void
TaskScheduler::DeleteArray(T* p_, size_t num_, const char* file_, int line_)
{
	...
	m_Config.customAllocator
	    .free(p_, sizeof(T) * num_, m_Config.customAllocator.userData, file_, line_);
}

template <class T, class... Args>
T*
TaskScheduler::New(const char* file_, int line_, Args&&... args_)
{
...
template <typename T>
void
TaskScheduler::Delete(T* p_, const char* file_, int line_)
{
	p_->~T();
...
template <typename T>
T*
TaskScheduler::Alloc(const char* file_, int line_)
{
	T* pRet = (T*)m_Config.customAllocator.alloc(
...
template <typename T>
void
TaskScheduler::Free(T* p_, const char* file_, int line_)
{
	m_Config.customAllocator
	    .free(p_, sizeof(T), m_Config.customAllocator.userData, file_, line_);
}
...
void
TaskScheduler::Initialize(uint32_t numThreadsTotal_)
{
...
void
TaskScheduler::Initialize(TaskSchedulerConfig config_)
{
	StopThreads(true); // Stops threads, waiting for them.
	m_Config = config_;
	StartThreads();
...
inline void
SemaphoreCreate(semaphoreid_t& semaphoreid)
{
#	ifdef _XBOX_ONE
...
inline void
SemaphoreClose(semaphoreid_t& semaphoreid)
{
	CloseHandle(semaphoreid.sem);
}

inline void
SemaphoreWait(semaphoreid_t& semaphoreid)
{
...
inline void
SemaphoreSignal(semaphoreid_t& semaphoreid, int32_t countWaiting)
{
	if (countWaiting) { ReleaseSemaphore(semaphoreid.sem, countWaiting, NULL); }
}
} // namespace candybox
...
SemaphoreSignal(semaphoreid_t& semaphoreid, int32_t countWaiting)
{
	while (countWaiting-- > 0) { dispatch_semaphore_signal(semaphoreid.sem); }
}
} // namespace candybox
...
SemaphoreSignal(semaphoreid_t& semaphoreid, int32_t countWaiting)
{
	while (countWaiting-- > 0) { sem_post(&semaphoreid.sem); }
}
} // namespace candybox
#endif

semaphoreid_t*
TaskScheduler::SemaphoreNew()
{
	semaphoreid_t* pSemaphore = this->Alloc<semaphoreid_t>(IE_FILE_AND_LINE);
	SemaphoreCreate(*pSemaphore);
	return pSemaphore;
}

void
TaskScheduler::SemaphoreDelete(semaphoreid_t* pSemaphore_)
{
	SemaphoreClose(*pSemaphore_);
	this->Free(pSemaphore_, IE_FILE_AND_LINE);
}

void
TaskScheduler::SetCustomAllocator(CustomAllocator customAllocator_)
{
...
void
Dependency::
    SetDependency(const ICompletable* pDependencyTask_, ICompletable* pTaskToRunOnCompletion_)
{
	ClearDependency();
...
Dependency::ClearDependency()
{
	if (pDependencyTask)
	{
		...
		--pTaskToRunOnCompletion->m_DependenciesCount;
		if (this == pDependent) { pDependencyTask->m_pDependents = pDependent->pNext; }
		else
		{
			while (pDependent)
			{
