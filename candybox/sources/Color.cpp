...
Color
candybox::RGBA(unsigned char r_, unsigned char g_, unsigned char b_, unsigned char a_)
{
	return {r_, g_, b_, a_};
}

Color
candybox::RGB_uc(unsigned char r_, unsigned char g_, unsigned char b_)
{
	return {r_, g_, b_};
}
...
{
	const float r = m::sin(t);
	const float g = m::sin(t + 0.33f * 2.0f * m::pi);
	const float b = m::sin(t + 0.66f * 2.0f * m::pi);
	return {r * r, g * g, b * b};
}
...
Color::Brighten(int amount)
{
	...
	r = m::max(0, m::min(255, r - (unsigned char)std::round(255 * -(amount / 100))));
	g = m::max(0, m::min(255, g - (unsigned char)std::round(255 * -(amount / 100))));
	b = m::max(0, m::min(255, b - (unsigned char)std::round(255 * -(amount / 100))));
	return *this;
}
...
Color
candybox::HSLA(float h, float s, float l, float a)
{
	float m1, m2, r, g, b;
	h = m::fmod(h, 1.0f);
	if (h < 0.0f) h += 1.0f;
	s = m::clamp(s, 0.0f, 1.0f);
	l = m::clamp(l, 0.0f, 1.0f);
	m2 = l <= 0.5f ? (l * (1 + s)) : (l + s - l * s);
	m1 = 2 * l - m2;
	r = m::clamp(Color::Hue(h + 1.0f / 3.0f, m1, m2), 0.0f, 1.0f);
	g = m::clamp(Color::Hue(h, m1, m2), 0.0f, 1.0f);
	b = m::clamp(Color::Hue(h - 1.0f / 3.0f, m1, m2), 0.0f, 1.0f);
	return {r, g, b, a};
}

Color
candybox::HSL(float h, float s, float l)
{
...
{
	a = (unsigned char)m::round(a_ * 255.f);
	return *this;
}
...
{
	r = (unsigned char)m::round(r_ * 255.0f);
	g = (unsigned char)m::round(g_ * 255.0f);
	b = (unsigned char)m::round(b_ * 255.0f);
	a = (unsigned char)m::round(a_ * 255.0f);
}
