...
static void
SiftUp(void *heap, uint32_t eleSize, uint32_t length, uint32_t pos, HeapIsSmallerCb isSmaller)
{
	...
	while (childPos < endPos)
	{
		...
		if (rightPos < endPos &&
		    !isSmaller(
		        priv::EleAt(heap, eleSize, childPos), priv::EleAt(heap, eleSize, rightPos)))
		{
			childPos = rightPos;
		}
...
{
	if (length == 1) { return priv::EleAt(heap, eleSize, 0); }
	else if (length > 1)
	{
