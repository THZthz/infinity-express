...
static inline float
getArea(glm::vec2 start, glm::vec2 end, glm::vec2 mid)
{
...
/// segment.
static float
perpendicularDistance(glm::vec2 start, glm::vec2 end, glm::vec2 mid, float sqSegLen)
{
	return m::pow2(getArea(start, end, mid)) / sqSegLen;
}

DeviationMetricCallback candybox::PerpDistMetric = &perpendicularDistance;

static float
shortestDistanceToSegment(glm::vec2 start, glm::vec2 end, glm::vec2 mid, float sqSegLen)
{
...
DeviationMetricCallback candybox::ShortestDistMetric = &shortestDistanceToSegment;
    ...
    DeviationMetricCallback deviationMetric)
{
	...
	*resPoints = *points;
	...
	if (errno || stack == nullptr) { COMPACT_PATH_RETURN(FAILURE); }
	...
	while (stackSize > 0)
	{
		if (!callStackPop(stack, &stackSize, &current)) { COMPACT_PATH_RETURN(FAILURE); }
		...
		else if (code == LINEARIZE || code == SOLVED)
		{
			...
			memmove(
			    resPoints + solvedPoints, current.resPoints, sizeof(glm::vec2) * validPoints);
    ...
    DeviationMetricCallback deviationMetric)
{
	...
	if (npoints < 3)
	{
		...
		if (npoints > 0) { memcpy(resPoints, points, sizeof(glm::vec2) * npoints); }
		*nResPoints = npoints;
		return SOLVED;
	}
