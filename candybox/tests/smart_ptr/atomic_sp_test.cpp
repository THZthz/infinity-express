...
main()
{
...

	{
		...
		boost::shared_ptr<X> p2 = apx.load(std::memory_order_acquire);
...

	{
		...
		boost::shared_ptr<X> px(new X);
		apx.store(px, std::memory_order_release);
...

	{
		...
		boost::shared_ptr<X> px(new X);
		boost::shared_ptr<X> p1 = apx.exchange(px, std::memory_order_acq_rel);
...

	{
		...
		bool r = apx.compare_exchange_weak(
		    cmp, px2, std::memory_order_seq_cst, std::memory_order_seq_cst);
		BOOST_TEST(!r);
		BOOST_TEST_SP_EQ(apx.load(), px);
		BOOST_TEST_SP_EQ(cmp, px);

		r = apx.compare_exchange_weak(
		    cmp, px2, std::memory_order_seq_cst, std::memory_order_seq_cst);
		BOOST_TEST(r);
		BOOST_TEST_SP_EQ(apx.load(), px2);
	}
...

	{
		...
		bool r = apx.compare_exchange_weak(
		    cmp, boost::shared_ptr<X>(), std::memory_order_seq_cst, std::memory_order_seq_cst);
...

	{
		...
		bool r = apx.compare_exchange_strong(
		    cmp, px2, std::memory_order_seq_cst, std::memory_order_seq_cst);
		BOOST_TEST(!r);
		BOOST_TEST_SP_EQ(apx.load(), px);
		BOOST_TEST_SP_EQ(cmp, px);

		r = apx.compare_exchange_strong(
		    cmp, px2, std::memory_order_seq_cst, std::memory_order_seq_cst);
		BOOST_TEST(r);
		BOOST_TEST_SP_EQ(apx.load(), px2);
	}
