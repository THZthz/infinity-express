...
bool
check_dynamic_pointer_cast(const BasePtr &ptr)
{
...
template <class BasePtr>
bool
check_static_pointer_cast(const BasePtr &ptr)
{
	return
	    //Cast base -> derived -> base2 using static_pointer_cast
	    boost::get_pointer(
	        boost::static_pointer_cast<base2>(boost::static_pointer_cast<derived>(ptr))) ==
...
template <class BasePtr>
bool
check_const_pointer_cast(const BasePtr &ptr)
{
	return
	    //Unconst and const again using const_pointer_cast
	    boost::get_pointer(
	        boost::const_pointer_cast<const base>(boost::const_pointer_cast<base>(ptr))) ==
...
template <class BasePtr>
void
check_all_copy_casts(const BasePtr &ptr)
{
...
template <class BasePtr>
bool
check_dynamic_moving_pointer_cast(std::function<BasePtr()> f)
{
	...
	return
	    //Correct cast with dynamic_pointer_cast
	    boost::get_pointer(boost::dynamic_pointer_cast<derived>(std::move(smart1))) ==
	        expect1 &&
	    //Incorrect cast with dynamic_pointer_cast
	    boost::get_pointer(boost::dynamic_pointer_cast<derived_derived>(std::move(smart2))) ==
	        expect2;
}
...
template <class BasePtr>
bool
check_static_moving_pointer_cast(std::function<BasePtr()> f)
{
	BasePtr smart = f();
	...
	return
	    //Cast base -> derived -> base2 using static_pointer_cast
	    boost::get_pointer(boost::static_pointer_cast<base2>(
...
template <class BasePtr>
bool
check_const_moving_pointer_cast(std::function<BasePtr()> f)
{
	BasePtr smart = f();
	const base
	    *expect = const_cast<const base *>(const_cast<base *>(boost::get_pointer(smart)));
	return
	    //Unconst and const again using const_pointer_cast
	    boost::get_pointer(boost::const_pointer_cast<const base>(
...
template <class BasePtr>
void
check_all_moving_casts(std::function<BasePtr()> f)
{
#if !defined(BOOST_NO_RTTI)
...
main()
{
	...
	check_all_moving_casts<base *>([plain]() { return plain; });

	// std::shared_ptr casts

	check_all_copy_casts(std_shared);
	check_all_moving_casts<std::shared_ptr<base>>([&std_shared]() { return std_shared; });
