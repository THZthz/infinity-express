...
test(T * = 0)
{
	...
	std::cout
	    << "   " << static_cast<double>(t3 - t) / CLOCKS_PER_SEC << " seconds, "
	    << static_cast<double>(t2 - t) / CLOCKS_PER_SEC << " + "
...
public:
	...
	void operator delete(void *p) { std::allocator<X>().deallocate(static_cast<X *>(p), 1); }
...
public:
	...
	void *operator new(std::size_t n) { return boost::detail::quick_allocator<Y>::alloc(n); }
