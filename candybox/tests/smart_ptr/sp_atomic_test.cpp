...
main()
{
...

	{
		...
		boost::shared_ptr<X> p3 = boost::atomic_exchange_explicit(
		    &px, boost::shared_ptr<X>(), std::memory_order_acq_rel);
		BOOST_TEST_SP_EQ(p3, px2);
		...
		bool r = boost::atomic_compare_exchange_explicit(
		    &px, &cmp, px4, std::memory_order_acquire, std::memory_order_relaxed);
		BOOST_TEST(!r);
		BOOST_TEST_SP_EQ(px, p2);
		BOOST_TEST_SP_EQ(cmp, p2);

		r = boost::atomic_compare_exchange_explicit(
		    &px, &cmp, px4, std::memory_order_release, std::memory_order_acquire);
		BOOST_TEST(r);
		BOOST_TEST_SP_EQ(px, px4);
	}
