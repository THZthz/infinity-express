...
struct deleter3
{
	void operator()(incomplete *p) { BOOST_TEST(p == 0); }
};
...
test()
{
	...
	{
		for (std::map<boost::shared_ptr<void>, long>::iterator i = m.begin(); i != m.end();
		     ++i)
		{
...
boost::shared_ptr<X>
make_shared_from_intrusive(X *p)
{
...
void
install_event_handler(boost::shared_ptr<event_handler> p)
{
	p->begin();
	current_handler = p;
}
...
test()
{
	...
	boost::shared_ptr<event_handler>
	    q(p.get(), smart_pointer_deleter<another_ptr<event_handler> >(p));
...
void
fread(boost::shared_ptr<file> pf, void *, long)
{
...
public:
	...
	void read(void *, long size) { total_size_ += size; }

	long total_size() const { return total_size_; }
};
