...
struct YD
{
	void operator()(Y* p) const
	{
		...
		else { BOOST_ERROR("YD::operator()(0) called"); }
	}
};

template <class U, class T, class D>
static void
test_null_unique_ptr(boost::movelib::unique_ptr<T, D> p1, boost::movelib::unique_ptr<T, D> p2)
{
...
main()
{
	...
	{
		test_null_unique_ptr<X>(
		    boost::movelib::unique_ptr<X>(), boost::movelib::unique_ptr<X>());
		test_null_unique_ptr<X const>(
		    boost::movelib::unique_ptr<X>(), boost::movelib::unique_ptr<X>());
		test_null_unique_ptr<void>(
		    boost::movelib::unique_ptr<X>(), boost::movelib::unique_ptr<X>());
		test_null_unique_ptr<void const>(
		    boost::movelib::unique_ptr<X>(), boost::movelib::unique_ptr<X>());
	}

	{
		test_null_unique_ptr<Y>(
		    boost::movelib::unique_ptr<Y, YD>(0, YD()),
		    boost::movelib::unique_ptr<Y, YD>(0, YD()));
		test_null_unique_ptr<Y const>(
		    boost::movelib::unique_ptr<Y, YD>(0, YD()),
		    boost::movelib::unique_ptr<Y, YD>(0, YD()));
		test_null_unique_ptr<void>(
		    boost::movelib::unique_ptr<Y, YD>(0, YD()),
