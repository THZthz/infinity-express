...
#ifndef CANDYBOX_LOCKLESS_MULTI_READ_PIPE_HPP__
#define CANDYBOX_LOCKLESS_MULTI_READ_PIPE_HPP__
...
template <uint8_t cSizeLog2, typename T>
inline bool
LockLessMultiReadPipe<cSizeLog2, T>::ReaderTryReadBack(T* pOut)
{
...
template <uint8_t cSizeLog2, typename T>
inline bool
LockLessMultiReadPipe<cSizeLog2, T>::WriterTryReadFront(T* pOut)
{
	uint32_t writeIndex = m_WriteIndex.load(std::memory_order_relaxed);
...
// Lockless multi-writer intrusive list
...
public:
	LocklessMultiWriteIntrusiveList() : pHead(&tail) { tail.pNext = nullptr; }
	...
	T* ReaderReadBack()
	{
		...
		if (pTailPlus1)
		{
			...
			else
			{
				tail.pNext = nullptr;
				T* pCompare =
				    pTailPlus1; // we need preserve pTailPlus1 as compare will alter it on failure
				...
				if (!pHead.compare_exchange_strong(pCompare, &tail))
				{
					...
					// pTailPlus1 is no longer the head, so pTailPlus1->pNext should be non nullptr
					while ((T*)nullptr == pTailPlus1->pNext)
					{
						;
					} // wait for pNext to be updated as head may have just changed.
					tail.pNext = pTailPlus1->pNext.load();
					pTailPlus1->pNext = nullptr;
				}
			}
		}
		return pTailPlus1;
...
#endif // CANDYBOX_LOCKLESS_MULTI_READ_PIPE_HPP__
