#ifndef __CANDYBOX_EASING_HPP__
#define __CANDYBOX_EASING_HPP__
...
{
	t /= d / 2;
	if (t < 1) return c / 2 * m::pow(2, 10 * (t - 1)) + b;
	t--;
...
{
	t /= d / 2;
	if (t < 1) return -c / 2 * (m::sqrt(1 - t * t) - 1) + b;
	t -= 2;
...
{
	double t0 = t / d;
	double f = m::sin(13.0f * 2.f * m::pi * t0) * m::pow(2, 10 * (t0 - 1));
	return b + c * f;
}
...
{
	double t0 = t / d;
	double f = m::sin(-13.0f * 2.f * m::pi * (t0 + 1.0f)) * m::pow(2, -10 * t0) + 1.0f;
	return b + c * f;
}
...
InOutElastic(double t, double b, double c, double d)
{
	double t0 = t / d;
	double a = 2.0f * t0;
	if (t0 < 0.5f) return 0.5f * m::sin(13.0f * 2.f * m::pi * a) * m::pow(2, 10 * (a - 1));
	double f = 0.5f * (m::sin(-13.0f * 2.f * m::pi * a) * m::pow(2, -10 * (a - 1)) + 2);
	return f * c + b;
}
...
_OutBounce(double t)
{
	...
	else { return ((54.0f / 5.0f) * tt) - ((513.0f / 25.0f) * t) + (268.0f / 25.0f); }
}
} // namespace priv
