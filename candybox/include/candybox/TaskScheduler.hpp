...
#ifndef CANDYBOX_TASK_SCHEDULER_HPP__
#define CANDYBOX_TASK_SCHEDULER_HPP__
...
// IE_TS_TASK_PRIORITIES_NUM can be set from 1 to 5.
// 1 corresponds to effectively no priorities.
#ifndef IE_TS_TASK_PRIORITIES_NUM
#	define IE_TS_TASK_PRIORITIES_NUM 3
#endif

#ifndef IE_TS_API
#	if defined(_WIN32) && defined(IE_TS_BUILD_DLL)
// Building enkiTS as a DLL
#		define IE_TS_API __declspec(dllexport)
#	elif defined(_WIN32) && defined(IE_TS_DLL)
// Using enkiTS as a DLL
#		define IE_TS_API __declspec(dllimport)
#	elif defined(__GNUC__) && defined(IE_TS_BUILD_DLL)
// Building enkiTS as a shared library
#		define IE_TS_API __attribute__((visibility("default")))
#	else
#		define IE_TS_API
#	endif
#endif

// Define IE_CUSTOM_ALLOC_FILE_AND_LINE (at project level) to get file and line report in custom allocators,
// this is default in Debug - to turn off define IE_CUSTOM_ALLOC_NO_FILE_AND_LINE
#ifndef IE_CUSTOM_ALLOC_FILE_AND_LINE
#	if defined(_DEBUG) && !defined(IE_CUSTOM_ALLOC_NO_FILE_AND_LINE)
#		define IE_CUSTOM_ALLOC_FILE_AND_LINE
#	endif
#endif
...
IE_TS_API uint32_t GetNumHardwareThreads();

enum TaskPriority
{
	TASK_PRIORITY_HIGH = 0,
#if (IE_TS_TASK_PRIORITIES_NUM > 3)
	TASK_PRIORITY_MED_HI,
#endif
#if (IE_TS_TASK_PRIORITIES_NUM > 2)
	TASK_PRIORITY_MED,
#endif
#if (IE_TS_TASK_PRIORITIES_NUM > 4)
	TASK_PRIORITY_MED_LO,
#endif
#if (IE_TS_TASK_PRIORITIES_NUM > 1)
	TASK_PRIORITY_LOW,
#endif
	TASK_PRIORITY_NUM
};
...
public:
	...
	TaskSet(uint32_t setSize_, TaskSetFunction func_)
	    : ITaskSet(setSize_), m_Function(std::move(func_))
	{
	}
...
struct ProfilerCallbacks
{
	...
	ProfilerCallbackFunc
	    waitForTaskCompleteSuspendStart; // thread suspended waiting task completion
	ProfilerCallbackFunc waitForTaskCompleteSuspendStop; // thread unsuspended
};

// Custom allocator, set in TaskSchedulerConfig. Also see IE_CUSTOM_ALLOC_FILE_AND_LINE for file_ and line_
typedef void* (
    *AllocFunc)(size_t align_, size_t size_, void* userData_, const char* file_, int line_);
typedef void (
    *FreeFunc)(void* ptr_, size_t size_, void* userData_, const char* file_, int line_);
IE_TS_API void*
DefaultAllocFunc(size_t align_, size_t size_, void* userData_, const char* file_, int line_);
IE_TS_API void
DefaultFreeFunc(void* ptr_, size_t size_, void* userData_, const char* file_, int line_);
...
public:
	...
	IE_TS_API void Initialize(uint32_t numThreadsTotal_);

	// Initialize with advanced TaskSchedulerConfig settings. See TaskSchedulerConfig.
	IE_TS_API void Initialize(TaskSchedulerConfig config_);

	// Get config. Can be called before Initialize to get the defaults.
	IE_TS_API TaskSchedulerConfig GetConfig() const;
	...
	IE_TS_API void AddTaskSetToPipe(ITaskSet* pTaskSet_);

	// Thread 0 is main thread, otherwise use threadNum
	// Pinned tasks can be added from any thread
	IE_TS_API void AddPinnedTask(IPinnedTask* pTask_);
	...
	IE_TS_API void WaitforTask(
	    const ICompletable* pCompletable_,
	    candybox::TaskPriority priorityOfLowestToRun_ = TaskPriority(TASK_PRIORITY_NUM - 1));
	...
	IE_TS_API void WaitforAllAndShutdown();
	...
	IE_TS_API void WaitForNewPinnedTasks();
	...
	IE_TS_API uint32_t GetNumTaskThreads() const;
	...
	IE_TS_API uint32_t GetThreadNum() const;
	...
	IE_TS_API bool RegisterExternalTaskThread();
	...
	IE_TS_API bool RegisterExternalTaskThread(uint32_t threadNumToRegister_);

	// Call on a thread on which RegisterExternalTaskThread has been called to deregister that thread.
	IE_TS_API void DeRegisterExternalTaskThread();

	// Get the number of registered external task threads.
	IE_TS_API uint32_t GetNumRegisteredExternalTaskThreads();
...
private:
	...
	IE_TS_API void
	TaskComplete(ICompletable* pTask_, bool bWakeThreads_, uint32_t threadNum_);
	IE_TS_API void AddTaskSetToPipeInt(ITaskSet* pTaskSet_, uint32_t threadNum_);
	IE_TS_API void AddPinnedTaskInt(IPinnedTask* pTask_);
...
inline void
ICompletable::OnDependenciesComplete(TaskScheduler* pTaskScheduler_, uint32_t threadNum_)
{
...
inline void
ITaskSet::OnDependenciesComplete(TaskScheduler* pTaskScheduler_, uint32_t threadNum_)
{
	pTaskScheduler_->AddTaskSetToPipeInt(this, threadNum_);
}

inline void
IPinnedTask::OnDependenciesComplete(TaskScheduler* pTaskScheduler_, uint32_t threadNum_)
{
	(void)threadNum_;
...
{
	assert(GetIsComplete()); // this task is still waiting to run
...
inline void
ICompletable::SetDependency(Dependency& dependency_, const ICompletable* pDependencyTask_)
{
...
template <typename D, typename T, int SIZE>
void
ICompletable::SetDependenciesArr(D& dependencyArray_, const T (&taskArray_)[SIZE])
{
	static_assert(std::tuple_size<D>::value >= SIZE, "Size of dependency array too small");
	for (int i = 0; i < SIZE; ++i) { dependencyArray_[i].SetDependency(&taskArray_[i], this); }
}
template <typename D, typename T>
void
ICompletable::SetDependenciesArr(D& dependencyArray_, std::initializer_list<T*> taskpList_)
{
	assert(std::tuple_size<D>::value >= taskpList_.size());
	int i = 0;
	for (auto pTask : taskpList_) { dependencyArray_[i++].SetDependency(pTask, this); }
}
template <typename D, typename T, int SIZE>
void
ICompletable::SetDependenciesArr(D (&dependencyArray_)[SIZE], const T (&taskArray_)[SIZE])
{
	for (int i = 0; i < SIZE; ++i) { dependencyArray_[i].SetDependency(&taskArray_[i], this); }
}
template <typename D, typename T, int SIZE>
void
ICompletable::
    SetDependenciesArr(D (&dependencyArray_)[SIZE], std::initializer_list<T*> taskpList_)
{
...
template <typename D, typename T, int SIZE>
void
ICompletable::SetDependenciesVec(D& dependencyVec_, const T (&taskArray_)[SIZE])
{
	dependencyVec_.resize(SIZE);
	for (int i = 0; i < SIZE; ++i) { dependencyVec_[i].SetDependency(&taskArray_[i], this); }
}

template <typename D, typename T>
void
ICompletable::SetDependenciesVec(D& dependencyVec_, std::initializer_list<T*> taskpList_)
{
	dependencyVec_.resize(taskpList_.size());
	int i = 0;
	for (auto pTask : taskpList_) { dependencyVec_[i++].SetDependency(pTask, this); }
}
} // namespace candybox

#endif // CANDYBOX_TASK_SCHEDULER_HPP__
