#ifndef CANDYBOX_HILBERT_HPP__
#define CANDYBOX_HILBERT_HPP__
...
// Ported from C++ https://kgithub.com/rawrunprotected/hilbert_curves (public domain)
namespace candybox {
namespace hilbert {
...
static inline void
HilbertIndexToXY(uint32_t n, uint32_t i, uint32_t *x, uint32_t *y)
{
	i = i << (32 - 2 * n);
...
static inline uint32_t
HilbertXYToIndex(uint32_t n, uint32_t x, uint32_t y)
{
	x = x << (16 - n);
	y = y << (16 - n);
...
static const uint8_t mortonToHilbertTable[] = {
    48, 33, 27, 34, 47, 78, 28, 77, 66, 29, 51, 52, 65, 30, 72, 63, 76, 95, 75, 24,
    53, 54, 82, 81, 18, 3,  17, 80, 61, 4,  62, 15, 0,  59, 71, 60, 49, 50, 86, 85,
    84, 83, 5,  90, 79, 56, 6,  89, 32, 23, 1,  94, 11, 12, 2,  93, 42, 41, 13, 14,
    35, 88, 36, 31, 92, 37, 87, 38, 91, 74, 8,  73, 46, 45, 9,  10, 7,  20, 64, 19,
...
static inline uint32_t
TransformCurve(uint32_t in, uint32_t bits, const uint8_t *lookupTable)
{
...
static inline uint32_t
MortonToHilbert3D(uint32_t mortonIndex, uint32_t bits)
{
	return TransformCurve(mortonIndex, bits, mortonToHilbertTable);
}

static inline uint32_t
HilbertToMorton3D(uint32_t hilbertIndex, uint32_t bits)
{
