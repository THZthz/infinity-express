#ifndef CANDYBOX_AABB_HPP__
#define CANDYBOX_AABB_HPP__
...
public:
	...
	float distance(const tvec &point) const { return m::sqrt(float(distanceSquare(point))); }
	...
	TBox getRotated(float rad) const
	{
		...
		return {m::subv(ce, newExtent), m::addv(ce, newExtent)};
	}
};
