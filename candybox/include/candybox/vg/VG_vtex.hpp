...
static float
glnvg__maxf(float a, float b)
{
	return a < b ? b : a;
}
static float
glnvg__minf(float a, float b)
{
	return a < b ? a : b;
}
static float
glnvg__clampf(float a, float mn, float mx)
{
...
static int
glnvg__mini(int a, int b)
{
	return a < b ? a : b;
}
static int
glnvg__clampi(int a, int mn, int mx)
{
	return a < mn ? mn : (a > mx ? mx : a);
}
...
static int
glnvg__compileShader(GLuint shader, const char* source[], int nsource)
{
	GLint status;
	...
	if (status != GL_TRUE)
	{
		...
		for (ii = 0; ii < nsource; ++ii) NVG_LOG(source[ii]);
#endif
		return 0;
	}
	return 1;
    ...
    int nfragsrc)
{
	...
	if (glnvg__compileShader(shader->vert, vertsrc, nvertsrc) == 0) return 0;
	if (glnvg__compileShader(shader->frag, fragsrc, nfragsrc) == 0) return 0;
...
\n vec2 pos_ex = va_in;
		\n gl_Position = vec4(
		    2.0f * pos_ex.x / viewSize.x - 1.0f, 1.0f - 2.0f * pos_ex.y / viewSize.y, 0.0f,
		    1.0f);
		\n
	});
...
\n  }
	...
	if (glnvg__createProgram(&gl->shader, vertsrc, 2, fragsrc, 2) == 0) return 0;
    ...
    const void* data)
{
	...
	else
	{
		...
		glTexImage2D(
		    GL_TEXTURE_2D, 0, GL_LUMINANCE, w, h, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, data);
#else
	...
	mipfilt =
	    imageFlags & NVG_IMAGE_NEAREST ? GL_NEAREST_MIPMAP_NEAREST : GL_LINEAR_MIPMAP_LINEAR;
...
#if !defined(NANOVG_GL2)
	if (imageFlags & NVG_IMAGE_GENERATE_MIPMAPS) { glGenerateMipmap(GL_TEXTURE_2D); }
#endif
...
static int
glnvg__renderUpdateTexture(void* uptr, int image, int x, int y, int w, int h, const void* data)
{
	GLNVGcontext* gl = (GLNVGcontext*)uptr;
	...
	if (tex->type == NVG_TEXTURE_RGBA) data += y * tex->width * 4;
	else data += y * tex->width;
	x = 0;
	w = tex->width;
#endif
...
glnvg__setUniforms(GLNVGcontext* gl, int uniformOffset, int image)
{
	...
	else { glnvg__bindTexture(gl, 0); }
}

static void
glnvg__renderViewport(void* uptr, float width, float height, float devicePixelRatio)
{
	GLNVGcontext* gl = (GLNVGcontext*)uptr;
...
glnvg__renderFlush(void* uptr)
{
	...
	NVG_LOG(
	    "renderFlush: %d calls, %d uniform blocks, %d edges, %.0f pixels, %.0f edges*pixels\n",
	...
	glBufferData(
	    GL_UNIFORM_BUFFER, gl->nuniforms * gl->fragSize, gl->uniforms, GL_STREAM_DRAW);
#endif
	...
	glVertexAttribPointer(
	    0, 2, GL_FLOAT, GL_FALSE, sizeof(NVGvertex), (const GLvoid*)(size_t)0);
	glVertexAttribPointer(
	    1, 2, GL_FLOAT, GL_FALSE, sizeof(NVGvertex),
	    (const GLvoid*)(0 + 2 * sizeof(float))); //offsetof(NVGVertex, x1)
	...
	glBindTexture(GL_TEXTURE_2D_ARRAY, gl->texEdges);
	glTexImage3D(
	    GL_TEXTURE_2D_ARRAY, 0, GL_RGBA32F, TILE_TEX_WIDTH, TILE_TEX_WIDTH, ntilelayers, 0,
	    GL_RGBA, GL_FLOAT, gl->edges);
	...
	for (i = 0; i < gl->ncalls; i++)
	{
		...
		if (i == 0 || call->uniformOffset != gl->calls[i - 1].uniformOffset ||
		    call->image != gl->calls[i - 1].image)
			glnvg__setUniforms(gl, call->uniformOffset, call->image);
		if (!prevblend || blend->srcRGB != prevblend->srcRGB ||
		    blend->srcAlpha != prevblend->srcAlpha || blend->dstRGB != prevblend->dstRGB ||
		    blend->dstAlpha != prevblend->dstAlpha)
			glnvg__blendFuncSeparate(gl, &call->blendFunc);
		glUniform1i(gl->shader.loc[GLNVG_LOC_TYPE], call->fragType);
		glUniform1i(
		    gl->shader.loc[GLNVG_LOC_NEDGES],
		    call->fillCount); // will be zero except for GLNVG_FILL
		glUniform1i(
		    gl->shader.loc[GLNVG_LOC_OFFSET],
		    call->fillOffset); // avoid problems due to undef uniforms
...
static int
glnvg__allocCalls(GLNVGcontext* gl, int n)
{
...
static int
glnvg__allocEdges(GLNVGcontext* gl, int n)
{
...
glnvg__allocFragUniforms(GLNVGcontext* gl, int n)
{
	...
	if (gl->nuniforms + n > gl->cuniforms)
	{
		...
		unsigned char* uniforms = (unsigned char*)
		    realloc(gl->uniforms, structSize * cuniforms);
...
static void
glnvg__vset2(NVGvertex* vtx, float x0, float y0, float x1, float y1) //, float u, float v)
{
	vtx->x0 = x0;
	vtx->y0 = y0;
	vtx->x1 = x1;
...
static NVGLcolor
glnvg__convertColor(GLNVGcontext* gl, NVGcolor c)
{
	NVGLcolor cout;
	cout.a = c.a / 255.0f;
    ...
    int flags)
{
	...
	if (paint->image != 0)
	{
		...
		else { nvgTransformInverse(invxform, paint->xform); }
	...
	if (offset > 0 &&
	    memcmp(frag, nvg__fragUniformPtr(gl, offset - gl->fragSize),
...
static void
glnvg__quad(NVGvertex* quad, const float* bounds, float pad)
{
...
glnvg__renderFill(
    void* uptr,
    NVGpaint* paint,
    NVGcompositeOperationState compOp,
    NVGscissor* scissor,
    int flags,
    const float* bounds,
    const NVGpath* paths,
    int npaths)
{
	...
	if (npaths == 1 && paths[0].convex && paths[0].nfill > 2) flags |= NVG_PATH_CONVEX;
	    ...
	    ncalls == 1 && nedges > 16 && (callw > 2 * gl->tilesize || callh > 2 * gl->tilesize))
	{
		...
		for (i = 0; i < npaths; i++)
		{
			...
			int pymin = glnvg__clampi(
			    ((int)(path->bounds[1] - ltrb[1] - 0.5f)) / tileh, 0, ytiles - 1);
			for (int j = 0; j < path->nfill; ++j)
			{
				float x0 = path->fill[j].x0, y0 = path->fill[j].y0, x1 = path->fill[j].x1,
				      y1 = path->fill[j].y1;
				...
				int vxmin = glnvg__clampi(
				    ((int)(glnvg__minf(x0, x1) - ltrb[0] - 0.5f)) / tilew, 0, xtiles - 1);
				int vxmax = glnvg__clampi(
				    ((int)(glnvg__maxf(x0, x1) - ltrb[0] + 0.5f)) / tilew, 0, xtiles - 1);
				int vymax = glnvg__clampi(
				    ((int)(glnvg__maxf(y0, y1) - ltrb[1] + 0.5f)) / tileh, 0, ytiles - 1);
				...
				for (int ix = vxmin; ix <= vxmax; ++ix)
				{
					for (int iy = pymin; iy <= vymax; ++iy)
					{
						...
						if (tile->nedges + 1 > tile->cedges)
						{
							...
							tile->edges = (float*)
							    realloc(tile->edges, 4 * sizeof(float) * tile->cedges);
						}
						...
						if (tile->nedges > 0)
						{
							...
							if (y0 > tymax && y1 > tymax && vtx[-3] > tymax && x0 == vtx[-2] &&
							    y0 == vtx[-1])
							{
								vtx[-2] = x1;
								vtx[-1] = y1;
		...
		for (int ix = 0; ix < xtiles; ++ix)
		{
			for (int iy = 0; iy < ytiles; ++iy)
			{
				...
				float tilebounds[4] = {
				    ltrb[0] + ix * tilew, ltrb[1] + iy * tileh,
				    glnvg__minf(ltrb[2], ltrb[0] + (ix + 1) * tilew),
				    glnvg__minf(ltrb[3], ltrb[1] + (iy + 1) * tileh)};
				memcpy(&gl->edges[edgeidx], tile->edges, sizeof(NVGvertex) * tile->nedges);
				if (callidx != call0) memcpy(call, &gl->calls[call0], sizeof(GLNVGcall));
...
#	endif
	...
	for (i = 0; i < gl->ntiles; ++i) free(gl->tiles[i].edges);
	free(gl->tiles);
	free(gl->textures);
	free(gl->edges);
	free(gl->verts);
...
void
nvglDelete(NVGcontext* ctx)
{
	nvgDeleteInternal(ctx);
}

int
nvglCreateImageFromHandle(NVGcontext* ctx, GLuint textureId, int w, int h, int imageFlags)
{
...
GLuint
nvglImageHandle(NVGcontext* ctx, int image)
{
	GLNVGcontext* gl = (GLNVGcontext*)nvgInternalParams(ctx)->userPtr;
	GLNVGtexture* tex = glnvg__findTexture(gl, image);
	return tex->tex;
