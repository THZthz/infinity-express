...
swnvg__sRGBLUTCalc()
{
	...
	for (i = 0; i < LINEAR_TO_SRGB_DIV + 1; ++i)
		linearToSRGB[i] =
		    (unsigned char)(0.5f + powf(i / ((float)LINEAR_TO_SRGB_DIV), 1 / sRGBgamma) * 255);
}

static SWNVGmemPage*
swnvg__nextPage(SWNVGthreadCtx* r, SWNVGmemPage* cur)
{
	SWNVGmemPage* newp;
	...
	// Add to linked list
	if (cur != NULL) cur->next = newp;
	else r->pages = newp;
	return newp;
}

static void
swnvg__resetPool(SWNVGthreadCtx* r)
{
...
swnvg__alloc(SWNVGthreadCtx* r, int size)
{
	unsigned char* buf;
...
static void
swnvg__addEdge(SWNVGcontext* r, NVGvertex* vtx)
{
	SWNVGedge* e;
...
static SWNVGactiveEdge*
swnvg__addActive(SWNVGthreadCtx* r, SWNVGedge* e, float startPoint)
{
	SWNVGactiveEdge* z;
	...
	if (dxdy < 0) z->dx = (int)(-floorf(SWNVG__FIX * -dxdy));
	else z->dx = (int)floorf(SWNVG__FIX * dxdy);
...
static void
swnvg__freeActive(SWNVGthreadCtx* r, SWNVGactiveEdge* z)
{
	z->next = r->freelist;
	r->freelist = z;
}
...
static void
swnvg__fillScanlineAA(unsigned char* scanline, int len, int x0, int x1, int i, int j)
{
	...
	if (i == j)
	{
		...
		scanline[i] =
		    (unsigned char)(scanline[i] + ((x1 - x0) * maxWeight >> SWNVG__FIXSHIFT));
	}
	else
	{
		if (i >= 0) // add antialiasing for x0
			scanline[i] =
			    (unsigned char)(scanline[i] +
			                    (((SWNVG__FIX - (x0 & SWNVG__FIXMASK)) * maxWeight) >>
			                     SWNVG__FIXSHIFT));
		else i = -1; // clip
		...
		if (j < len) // add antialiasing for x1
			scanline[j] =
			    (unsigned char)(scanline[j] +
			                    (((x1 & SWNVG__FIXMASK) * maxWeight) >> SWNVG__FIXSHIFT));
		else j = len; // clip
...
static void
swnvg__fillScanline(unsigned char* scanline, int len, int x0, int x1, int i, int j)
{
	...
	if (i == j)
	{
		...
		scanline[i] =
		    (x0 & SWNVG__FIXMASK) <= SWNVG__FIX / 2 && (x1 & SWNVG__FIXMASK) > SWNVG__FIX / 2
		        ? maxWeight
		        : 0;
	}
	else
	{
		if (i >= 0) scanline[i] = (x0 & SWNVG__FIXMASK) <= SWNVG__FIX / 2 ? maxWeight : 0;
		else i = -1; // clip

		if (j < len) scanline[j] = (x1 & SWNVG__FIXMASK) > SWNVG__FIX / 2 ? maxWeight : 0;
		else j = len; // clip
...
static void
swnvg__fillActiveEdges(SWNVGthreadCtx* r, SWNVGactiveEdge* e, int* xmin, int* xmax, int flags)
{
		...
		else
		{
			...
			if (w == 0)
			{
				...
				if (i <= right && j >= left)
				{
					if (i < *xmin) *xmin = i;
					if (j > *xmax) *xmax = j;
					(flags & NVG_PATH_NO_AA)
					    ? swnvg__fillScanline(scanline, len, x0, x1, i - left, j - left)
...
static float
swnvg__lengthf(float x, float y)
{
	return sqrtf(x * x + y * y);
}
static float
swnvg__maxf(float a, float b)
{
	return a < b ? b : a;
}
static float
swnvg__minf(float a, float b)
{
	return a < b ? a : b;
}
static float
swnvg__clampf(float a, float mn, float mx)
{
	return a < mn ? mn : (a > mx ? mx : a);
}
static int
swnvg__maxi(int a, int b)
{
	return a < b ? b : a;
}
static int
swnvg__mini(int a, int b)
{
	return a < b ? a : b;
}
static int
swnvg__clampi(int a, int mn, int mx)
{
	return a < mn ? mn : (a > mx ? mx : a);
}
...
static void
swnvg__blendSRGB(unsigned char* dst, int cover, int cr, int cg, int cb, int ca)
{
...
static void
swnvg__blendLinear(unsigned char* dst, int cover, int cr, int cg, int cb, int ca)
{
	...
	// write to bitmap
	dst[0] = linearToSRGB
	    [r]; // swnvg__mini(r, 2047)];  -- clamping only needed for premultiplication w/
...
static void
swnvg__blend(unsigned char* dst, int cover, int cr, int cg, int cb, int ca, int linear)
{
	linear ? swnvg__blendLinear(dst, cover, cr, cg, cb, ca)
	       : swnvg__blendSRGB(dst, cover, cr, cg, cb, ca);
}

static void
swnvg__blend8888(unsigned char* dst, int cover, int cr, int cg, int cb, int ca, int linear)
{
	if (cover == 255 && ca == 255)
	{
	...
	else swnvg__blend(dst, cover, cr, cg, cb, ca, linear);
}

// assumes color is opaque
static void
swnvg__blendOpaque(unsigned char* dst, int cover, rgba32_t rgba, int linear)
{
	if (cover == 255) *(rgba32_t*)dst = rgba;
	else
		swnvg__blend(
		    dst, cover, COLOR0(rgba), COLOR1(rgba), COLOR2(rgba), COLOR3(rgba), linear);
}

static rgba32_t
texelFetchRGBA32(SWNVGtexture* tex, int x, int y)
{
...
static unsigned int
swnvg__mix8(float fx, float fy, int t00, int t10, int t01, int t11)
{
...
swnvg__lerpAndBlend(
    ...
    int linear)
{
	ijx = swnvg__maxf(0.0f, ijx);
	ijy = swnvg__maxf(0.0f, ijy);
	int ij00x = swnvg__mini((int)ijx, tex->width - 1),
	    ij00y = swnvg__mini((int)ijy, tex->height - 1);
	int ij11x = swnvg__mini((int)ijx + 1, tex->width - 1),
	    ij11y = swnvg__mini((int)ijy + 1, tex->height - 1);
    ...
    SWNVGcall* call)
{
	...
	if (call->type == SWNVG_PAINT_COLOR)
	{
		...
		{
			for (i = 0; i < count; ++i, dst += 4) swnvg__blendOpaque(dst, *cover++, c, linear);
		}
		else
		{
			for (i = 0; i < count; ++i, dst += 4)
				swnvg__blend(
				    dst, *cover++, COLOR0(c), COLOR1(c), COLOR2(c), COLOR3(c), linear);
		}
	}
	else if (call->type == SWNVG_PAINT_IMAGE)
	{
		...
		nvgTransformPoint(&qx, &qy, call->paintMat, (float)x, (float)y);
		...
		for (i = 0; i < count; ++i)
		{
			...
			{
				int imgx = swnvg__clampi((int)(0.5f + (float)qx), 0, call->tex->width - 1);
				int imgy = swnvg__clampi((int)(0.5f + (float)qy), 0, call->tex->height - 1);
				rgba32_t c = img[imgy * call->tex->width + imgx];
				if (RGBA32_IS_OPAQUE(c)) swnvg__blendOpaque(dst, *cover++, c, linear);
				else
					swnvg__blend(
					    dst, *cover++, COLOR0(c), COLOR1(c), COLOR2(c), COLOR3(c), linear);
			}
			else swnvg__lerpAndBlend(dst, *cover++, call->tex, qx, qy, linear);
	...
	else if (call->type == SWNVG_PAINT_GRAD)
	{
		...
		for (i = 0; i < count; ++i)
		{
			...
			float d0 =
			    swnvg__minf(swnvg__maxf(dx, dy), 0.0f) +
...
static void
swnvg__rasterizeSortedEdges(SWNVGthreadCtx* r, SWNVGcall* call)
{
	...
	for (y = swnvg__maxi(r->y0, call->bounds[1]); y <= swnvg__mini(r->y1, call->bounds[3]);
	     y++)
	{
		xmin = gl->width;
		xmax = 0;
		for (s = 0; s < SWNVG__SUBSAMPLES; ++s)
		{
			...
			if ((call->flags & NVG_PATH_NO_AA) && s != SWNVG__SUBSAMPLES / 2) continue;
			...
			if (active != NULL) swnvg__fillActiveEdges(r, active, &xmin, &xmax, call->flags);
		}
		...
		if (xmin1 <= xmax1)
			swnvg__scanlineSolid(
			    &gl->bitmap[y * gl->stride + xmin1 * 4], xmax1 - xmin1 + 1,
			    &r->scanline[xmin1 - r->x0], xmin1, y, call);
		// we fill x range clipped to scissor, but we have to clear entire range written by fillActiveEdges
		if (xmin <= xmax) memset(&r->scanline[xmin - r->x0], 0, xmax - xmin + 1);
	}
}

static float
texFetchF32(SWNVGtexture* tex, int x, int y)
{
...
texFetchF32Lerp(
    ...
    int ijmaxy)
{
	int ij00x = swnvg__clampi((int)ijx, ijminx, ijmaxx),
	    ij00y = swnvg__clampi((int)ijy, ijminy, ijmaxy);
	int ij11x = swnvg__clampi((int)ijx + 1, ijminx, ijmaxx),
	    ij11y = swnvg__clampi((int)ijy + 1, ijminy, ijmaxy);
...
static unsigned char
texFetch(SWNVGtexture* tex, int x, int y)
{
...
static float
sdfCov(float D, float sdfscale, float sdfoffset)
{
	return D > 0.0f ? swnvg__clampf((D - 255.0f * 0.5f) / sdfscale + sdfoffset, 0.0f, 1.0f)
	                : 0.0f;
}
    ...
    int ijmaxy)
{
	...
	float sd =
	    (d - 255.0f * 0.5f) / s +
	    (dr - 0.5f); // note we're still using the half pixel scale here
	...
	return 0.25f *
	       (sdfCov(d11, s, dr) + sdfCov(d10, s, dr) + sdfCov(d01, s, dr) + sdfCov(d00, s, dr));
}

static void
swnvg__rasterizeQuad(
    SWNVGthreadCtx* r,
    SWNVGcall* call,
    SWNVGtexture* tex,
    NVGvertex* v00,
    NVGvertex* v11)
{
	...
	float sdfscale =
	    0.5f * 32.0f * call->paintMat[0]; // 0.5 - we're sampling 4 0.5x0.5 subpixels
	...
	int ijminx = ((int)(s00 / extentx + 0.5f)) * extentx,
	    ijminy = ((int)(t00 / extenty + 0.5f)) * extenty;
	...
	for (y = ymin; y <= ymax; ++y)
	{
		...
		{
#ifdef FONS_SDF
			float cover = superSDF(
			    tex, sdfscale, sdfoffset, s, t, ds / 2, dt / 2, ijminx, ijminy, ijmaxx,
			    ijmaxy);
#else
...
static SWNVGtexture*
swnvg__allocTexture(SWNVGcontext* gl)
{
	...
	if (tex == NULL)
	{
		if (gl->ntextures + 1 > gl->ctextures)
		{
			SWNVGtexture* textures;
			int ctextures =
			    swnvg__maxi(gl->ntextures + 1, 4) + gl->ctextures / 2; // 1.5x Overallocate
...
static SWNVGtexture*
swnvg__findTexture(SWNVGcontext* gl, int id)
{
	int i;
	for (i = 0; i < gl->ntextures; i++)
...
static void
swnvg__copyRGBAData(SWNVGcontext* gl, SWNVGtexture* tex, const void* data)
{
	int ii;
	...
	if (tex->flags & NVG_IMAGE_PREMULTIPLIED)
	{
		...
		for (ii = 0; ii < npix; ++ii, ++dest, ++src)
		{
			...
			int b = COLOR2(*src);
			int a = COLOR3(*src);
			*dest = ((255 * r) / a << gl->rshift | (255 * g) / a << gl->gshift |
			         (255 * b) / a << gl->bshift | a << gl->ashift);
		}
	}
    ...
    const void* data)
{
	...
	tex->type = type;
	if (imageFlags &
	    NVG_IMAGE_NOCOPY) // we'll require user to make sure image byte order matches framebuffer
		tex->data = (void*)data;
	else
	{
		...
		if (!data) { }
		else if (tex->type == NVG_TEXTURE_RGBA) swnvg__copyRGBAData(gl, tex, data);
		else memcpy(tex->data, data, nbytes);
	}
	return tex->id;
}

static int
swnvg__renderDeleteTexture(void* uptr, int image)
{
	SWNVGcontext* gl = (SWNVGcontext*)uptr;
	SWNVGtexture* tex = swnvg__findTexture(gl, image);
	if (!tex) return 0;
	if (!(tex->flags & NVG_IMAGE_NOCOPY)) free(tex->data);
	memset(tex, 0, sizeof(SWNVGtexture));
	return 1;
}

static int
swnvg__renderUpdateTexture(void* uptr, int image, int x, int y, int w, int h, const void* data)
{
	SWNVGcontext* gl = (SWNVGcontext*)uptr;
	SWNVGtexture* tex = swnvg__findTexture(gl, image);
	...
	else
	{
		...
		memcpy((char*)tex->data + dy, (const char*)data + dy,
		       tex->width * h * nb); // no support for partial width
	}
	return 1;
}

static int
swnvg__renderGetTextureSize(void* uptr, int image, int* w, int* h)
{
	SWNVGcontext* gl = (SWNVGcontext*)uptr;
	SWNVGtexture* tex = swnvg__findTexture(gl, image);
	if (!tex) return 0;
...
static void
swnvg__renderViewport(void* uptr, float width, float height, float devicePixelRatio)
{
}

static void
swnvg__renderCancel(void* uptr)
{
	SWNVGcontext* gl = (SWNVGcontext*)uptr;
	gl->nverts = 0;
	gl->nedges = 0;
...
static void
swnvg__addEdgeXC(SWNVGcontext* r, NVGvertex* vtx, float xmax)
{
	SWNVGedge* e;
	// Skip horizontal edges
	if (vtx->y0 == vtx->y1) return;
...
static float
areaEdge2(float v0x, float v0y, float v1x, float v1y, float slope)
{
	...
	if (slope == 0) return width * swnvg__clampf(0.5f - v0y, 0.0f, 1.0f);
	float midx = 0.5f * (win0 + win1);
	float y = v0y + (midx - v0x) * slope; // y value at middle of window
	float dy = fabsf(slope * width);
	float sx = swnvg__clampf(
	    y + 0.5f * dy + 0.5f, 0.0f, 1.0f); // shift from -0.5..0.5 to 0..1 for area calc
	...
	float area =
	    0.5f *
	    (sz - sz * sy + 1.0f - sx +
	     sx * sw); // +1.0 for fill in +y direction, -1.0 for -y direction
	return area * width;
}

static void
swnvg__rasterizeXC(SWNVGthreadCtx* r, SWNVGcall* call)
{
	...
	for (i = 0; i < call->edgeCount; ++i, ++edge)
	{
		...
		for (iy = iymin; iy <= iymax; ++iy)
		{
			...
			{
				float c = areaEdge2(
				    edge->y0 - iy - 0.5f, edge->x0 - ix - 0.5f, edge->y1 - iy - 0.5f,
				    edge->x1 - ix - 0.5f, invslope);
				*dst++ += c - cov;
				cov = c;
			}
			...
			// scanline x limits
			if (ixmin < lims[0]) lims[0] = ixmin;
			if (ixmax >= lims[1]) lims[1] = ixmax + 1;
			lims += 2;
			xmin += invslope;
			xmax += invslope;
		}
	...
	for (iy = yb0; iy <= yb1; ++iy)
	{
		...
		lims[0] = gl->width;
		lims[1] = 0; // reset limits for this scanline
		lims += 2;
		...
		if (call->type == SWNVG_PAINT_COLOR)
		{
			...
			if (RGBA32_IS_OPAQUE(c))
			{
				for (i = 0; i < count; ++i, ++dcover, dst += 4)
				{
					...
					if (icover > 0) swnvg__blendOpaque(dst, icover, c, linear);
				}
			}
			else
			{
				for (i = 0; i < count; ++i, ++dcover, dst += 4)
				{
					...
					if (icover > 0)
						swnvg__blend(
						    dst, icover, COLOR0(c), COLOR1(c), COLOR2(c), COLOR3(c), linear);
				}
			}
		}
		else
...
static void
swnvg__insSortEdges(SWNVGedge* p, int n)
{
	int i, j;
	for (i = 1; i < n; ++i)
	{
...
static void
swnvg__quickSortEdges(SWNVGedge* p, int n)
{
...
static void
swnvg__sortCallEdges(SWNVGedge* p, int n)
{
...
swnvg__rasterize(void* arg)
{
	...
	{
		SWNVGcall* call = &gl->calls[i];
		if (call->bounds[0] <= r->x1 && call->bounds[1] <= r->y1 && call->bounds[2] >= r->x0 &&
		    call->bounds[3] >= r->y0)
		{
			...
			{
				if (call->flags & NVG_PATH_XC) swnvg__rasterizeXC(r, call);
				else
				{
					swnvg__resetPool(r);
					r->freelist = NULL;
...
swnvg__renderFlush(void* uptr)
{
	...
	if (nthreads > 1)
	{
		for (i = 0; i < nthreads; ++i) gl->poolSubmit(swnvg__sortEdges, &gl->threads[i]);
		gl->poolWait();
		for (i = 0; i < nthreads; ++i) gl->poolSubmit(swnvg__rasterize, &gl->threads[i]);
		gl->poolWait();
	}
	else
	{
...
static SWNVGcall*
swnvg__allocCall(SWNVGcontext* gl)
{
	SWNVGcall* ret = NULL;
...
static int
swnvg__allocVerts(SWNVGcontext* gl, int n)
{
	int ret = 0;
...
static rgba32_t
swnvg__convertColor(SWNVGcontext* gl, NVGcolor c)
{
    ...
    int flags)
{
	...
	call->bounds[0] = 0;
	call->bounds[1] = 0;
	call->bounds[2] = gl->width - 1;
	call->bounds[3] = gl->height - 1;
	if (scissor->extent[0] > -0.5f && scissor->extent[1] > -0.5f)
	{
		...
		else NVG_LOG("nanovg_sw only supports axis aligned scissor!\n");
#endif
	}
	...
	else { call->type = SWNVG_PAINT_COLOR; }
	return 1;
}
    ...
    int npaths)
{
	...
	int i, j, maxverts = 0;
	int ibounds[4] = {
	    (int)bounds[0], (int)bounds[1], (int)(ceilf(bounds[2])), (int)(ceilf(bounds[3]))};
	SWNVGcall* call = NULL;
	for (i = 0; i < npaths; ++i) maxverts += paths[i].nfill;
	if (maxverts == 0) return;
	call = swnvg__allocCall(gl);
	if (call == NULL) return;

	swnvg__convertPaint(gl, call, paint, scissor, flags);
	if ((gl->flags & NVGSW_PATHS_XC) && !(call->flags & NVG_PATH_NO_AA) &&
	    !(call->flags & NVG_PATH_EVENODD))
	{
	...
	for (i = 0; i < npaths; ++i)
	{
		...
		for (j = 0; j < path->nfill; ++j)
			...
			else swnvg__addEdge(gl, &path->fill[j]);
	}
	call->edgeCount = gl->nedges - call->edgeOffset;
}

static void
swnvg__renderTriangles(
    void* uptr,
    NVGpaint* paint,
    NVGcompositeOperationState compositeOperation,
    NVGscissor* scissor,
