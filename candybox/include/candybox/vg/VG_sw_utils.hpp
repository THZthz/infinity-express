...
static void
checkGLError(const char* str)
{
	GLenum err;
	while ((err = glGetError()) != GL_NO_ERROR) NVG_LOG("Error 0x%08x after %s\n", err, str);
}
...
void
nvgswuDeleteBlitter(NVGSWUblitter* ctx)
{
	if (!ctx) return;
...
void
nvgswuBlit(NVGSWUblitter* ctx, void* pixels, int width, int height, int x, int y, int w, int h)
{
	glActiveTexture(GL_TEXTURE0);
	if (width != ctx->width || height != ctx->height)
	{
		if (ctx->tex > 0) glDeleteTextures(1, &ctx->tex);
	...
	{ // update existing texture
		...
		glTexSubImage2D(
		    GL_TEXTURE_2D, 0, 0, y, ctx->width, h, GL_RGBA, GL_UNSIGNED_BYTE,
		    (uint8_t*)pixels + y * ctx->width * 4);
#else
	...
	glVertexAttribPointer(
	    1, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), (const GLvoid*)(2 * sizeof(GLfloat)));
