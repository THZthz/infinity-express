...
NVGcolor nvgRGBui(unsigned int rgb);
...
void nvgTransform(NVGcontext* ctx, const float* mat);
...
void nvgPolygon(NVGcontext* ctx, int count, float* vertices_);

void nvgSegment(NVGcontext* ctx, float x1, float y1, float x2, float y2);

void nvgCapsule(NVGcontext* ctx, float x1, float y1, float x2, float y2, float r);

void nvgRoundPolygon(NVGcontext* ctx, int count, float* verts_, float r);
...
enum NVGpathFlags
{
	...
	NVG_PATH_CONVEX = 1 << 2,
	NVG_PATH_XC =
	    1 << 3, // request exact coverage rendering for renderers w/ alternative options
};
