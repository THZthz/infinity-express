...
bool SegmentIntersection(const Segment& a, const Segment& b, bool infiniteLines, glm::vec2& out);
...
InvRotateVector(Rot2d q, glm::vec2 v)
{
...
static inline glm::vec2
TransformPoint(Xf2d xf, const glm::vec2 p)
{
...
// Rotate a vector
static inline glm::vec2
RotateVector(Rot2d q, glm::vec2 v)
{
	return {q.c * v.x - q.s * v.y, q.s * v.x + q.c * v.y};
}
...
static inline glm::vec2
InvTransformPoint(Xf2d xf, const glm::vec2 p)
{
	float vx = p.x - xf.p.x;
	float vy = p.y - xf.p.y;
