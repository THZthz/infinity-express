...
#	define NANOVG_GL_USE_UNIFORMBUFFER                                                       \
		0 // uniform buffer slower, at least on desktop and iPhone 6S
#endif
...
\n  }
	...
	{
		if (NVG_HAS_EXT(EXT_shader_framebuffer_fetch)) gl->renderMethod = NVGL_RENDER_FB_FETCH;
	}
#endif
...
#elif defined(                                                                                \
    GL_ES_VERSION_3_1) //defined(GL_OES_shader_image_atomic) || defined(GL_ES_VERSION_3_2)
		...
		gl->renderMethod = NVGL_RENDER_IMG_ATOMIC;
#else
#	define NVGL_NO_IMG_ATOMIC                                                                \
		1 // disable code if no possibility of support on current platform
#endif
	}
	...
	else { NVG_LOG("nvg2: no extensions in use\n"); }

	if (glnvg__createShader(
	        &gl->shader, "shader", shaderHeader, shaderOpts, fillVertShader, fillFragShader) ==
	    0)
		return 0;
...
glnvg__renderViewport(void* uptr, float width, float height, float devicePixelRatio)
{
	...
	{
		if (gl->texWinding > 0) glDeleteTextures(1, &gl->texWinding);
		if (gl->texColor > 0) glDeleteTextures(1, &gl->texColor);
		gl->texWinding = 0;
		gl->texColor = 0;
	}
	gl->view[0] = width;
...
glnvg__fill(GLNVGcontext* gl, GLNVGcall* call)
{
	...
	glnvg__setUniforms(
	    gl, call->uniformOffset, call->image); // NOTE: this binds/unbinds texture!
	...
	// accumulate winding
	if (gl->renderMethod == NVGL_RENDER_FB_SWAP) glBlendFunc(GL_ONE, GL_ONE);
	glUniform1i(gl->shader.loc[GLNVG_LOC_TYPE], NSVG_SHADER_WINDINGCALC);
	for (i = 0; i < npaths; i++)
	{
		glUniform1f(
		    gl->shader.loc[GLNVG_LOC_WINDINGYMIN], paths[i].yMin < 0 ? 0 : paths[i].yMin);
...
static void
glnvg__imgAtomicFlush(GLNVGcontext* gl)
{
	...
	while (fillidx < gl->ncalls)
	{
		for (; accumidx < gl->ncalls; ++accumidx)
		{
			...
			if (call->type == GLNVG_FILL)
			{
				...
				int i, npaths = call->pathCount;
				int disableAA =
				    (int)nvg__fragUniformPtr(gl, call->uniformOffset)->fillMode &
				            NVG_PATH_NO_AA
				        ? 1
				        : 0;
				// setup params for converting gl_FragCoord.xy to image index
				int w =
				    call->bounds[2] - call->bounds[0] + 1; // +1 because bounds are inclusive
				int minidx = call->bounds[0] + (int)((gl->view[1] - call->bounds[3] - 1) * w);
				int maxidx = call->bounds[2] + (int)((gl->view[1] - call->bounds[1] - 1) * w);
				...
				if (nextOffset > gl->imgWindingSize) break;
				...
				glUniform4i(
				    gl->shader.loc[GLNVG_LOC_IMGPARAMS], call->imgOffset, w, (int)gl->view[0],
				    disableAA);
				...
				{
					glUniform1f(
					    gl->shader.loc[GLNVG_LOC_WINDINGYMIN],
					    paths[i].yMin < 0 ? 0 : paths[i].yMin);
					glDrawArrays(GL_TRIANGLES, paths[i].fillOffset, paths[i].fillCount);
				}
				glnvg__checkError(gl, "fill: accumulate winding");
			}
...
glnvg__renderFlush(void* uptr)
{
	...
	if (gl->texWinding == 0)
	{
		...
		if (gl->renderMethod == NVGL_RENDER_IMG_ATOMIC)
...
#else
			glTexImage2D(
			    GL_TEXTURE_2D, 0, GL_R32I, (GLsizei)gl->view[0], (GLsizei)gl->view[1], 0,
			...
			glTexImage2D(
			    GL_TEXTURE_2D, 0, GL_R32F, (GLsizei)gl->view[0], (GLsizei)gl->view[1], 0,
		...
		glFramebufferTexture2D(
		    GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, gl->texWinding, 0);
		glFramebufferTexture2D(
	...
	if (gl->renderMethod == NVGL_RENDER_FB_FETCH)
	{
		...
		glFramebufferTexture2D(
		    GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, gl->texWinding, 0);
	...
	else if (gl->renderMethod == NVGL_RENDER_IMG_ATOMIC)
	{
		...
		gl->imgWindingOffset = 0;
		gl->imgWindingSize = (int)(gl->view[0] * gl->view[1]);
#endif
	}
	...
	if (gl->renderMethod == NVGL_RENDER_FB_FETCH) glDisable(GL_BLEND);
...
#ifndef NVGL_NO_IMG_ATOMIC
	if (gl->renderMethod == NVGL_RENDER_IMG_ATOMIC) glnvg__imgAtomicFlush(gl);
	else
#endif
	{
    ...
    int flags)
{
	...
	frag->fillMode = (float)flags; // NVGpathFlags
	...
	else { call->fragType = NSVG_SHADER_FILLSOLID; }
	return 1;
}
    ...
    int npaths)
{
	...
	else
	{
		...
		call->bounds[0] = glnvg__clampi((int)(bounds[0] - 0.001f), 0, (int)(gl->view[0] - 1));
		call->bounds[1] = glnvg__clampi((int)(bounds[1] - 0.001f), 0, (int)(gl->view[1] - 1));
		call->bounds[2] = glnvg__clampi((int)(bounds[2] + 1.001f), 0, (int)(gl->view[0] - 1));
		call->bounds[3] = glnvg__clampi((int)(bounds[3] + 1.001f), 0, (int)(gl->view[1] - 1));
