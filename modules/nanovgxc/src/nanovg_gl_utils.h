...
void
nvgluSetFramebufferSRGB(int enable)
{
...
void
nvgluSetFramebufferSize(NVGLUframebuffer* fb, int w, int h, int imageFlags)
{
	GLint internalfmt = imageFlags & NVG_IMAGE_SRGB ? GL_SRGB8_ALPHA8 : GL_RGBA8;
	if (w <= 0 || h <= 0 || (w == fb->width && h == fb->height)) return;
	if (fb->image >= 0)
	{
		NVG_LOG("nvgluSetFramebufferSize() can only be used with framebuffer created with "
		        "NVGLU_NO_NVG_IMAGE.");
		return;
	}
...
void
nvgluBlitFramebuffer(NVGLUframebuffer* fb, int destFBO)
{
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, destFBO);
	glBlitFramebuffer(
	    0, 0, fb->width, fb->height, 0, 0, fb->width, fb->height, GL_COLOR_BUFFER_BIT,
	    GL_NEAREST);
...
void
nvgluReadPixels(NVGLUframebuffer* fb, void* dest)
{
...
void
nvgluBindFBO(int fbo)
{
	glBindFramebuffer(GL_FRAMEBUFFER, fbo);
}

void
nvgluSetViewport(int x, int y, int w, int h)
{
	glViewport(x, y, w, h);
}

void
nvgluSetScissor(int x, int y, int w, int h)
{
	if (w <= 0 || h <= 0) glDisable(GL_SCISSOR_TEST);
	else
	{
...
int
nvgluGetImageHandle(NVGLUframebuffer* fb)
{
	return fb->image;
}
...
nvgluDeleteFramebuffer(NVGLUframebuffer* fb)
{
	...
	else glDeleteTextures(1, &fb->texture);
	fb->ctx = NULL;
	fb->fbo = 0;
	fb->texture = 0;
	fb->image = -1;
