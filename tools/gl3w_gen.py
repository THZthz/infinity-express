...
download('https://registry.khronos.org/OpenGL/api/GL/glcorearb.h', os.path.join(args.root, './glcorearb.h'))
download('https://registry.khronos.org/EGL/api/KHR/khrplatform.h', os.path.join(args.root, './khrplatform.h'))

# Parse function names from glcorearb.h
print('Parsing glcorearb.h header...')
procs = []
p = re.compile(r'GLAPI.*APIENTRY\s+(\w+)')
with open(os.path.join(args.root, './glcorearb.h'), 'r') as f:
    for line in f:
        m = p.match(line)
        if not m:
            continue
...
# Generate gl3w.h
print('Generating {0}...'.format(os.path.join(args.root, './gl3w.h')))
with open(os.path.join(args.root, './gl3w.h'), 'wb') as f:
    write(f, UNLICENSE)
