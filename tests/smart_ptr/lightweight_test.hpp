...
{
public:
	test_result() : report_(false), errors_(0) { core::detail::lwt_unattended(); }
	...
	~test_result()
	{
		if (!report_)
		{
			BOOST_LIGHTWEIGHT_TEST_OSTREAM
			    << "main() should return report_errors()" << std::endl;
			std::abort();
		}
	}
...
inline bool
test_impl(char const* expr, char const* file, int line, char const* function, bool v)
{
	if (v)
	{
		test_results();
...
inline void
error_impl(char const* msg, char const* file, int line, char const* function)
{
	BOOST_LIGHTWEIGHT_TEST_OSTREAM
	    << file << "(" << line << "): " << msg << " in function '" << function << "'"
	    << std::endl;
	++test_results().errors();
}
...
{
	BOOST_LIGHTWEIGHT_TEST_OSTREAM
	    << file << "(" << line << "): expression '" << expr << "' did not throw exception '"
	    << excep << "' in function '" << function << "'" << std::endl;
	++test_results().errors();
}

inline void
no_throw_failed_impl(const char* expr, const char* file, int line, const char* function)
{
	BOOST_LIGHTWEIGHT_TEST_OSTREAM
...
template <> struct lwt_long_type<true>
{
	typedef long type;
};
template <> struct lwt_long_type<false>
{
	typedef unsigned long type;
};

inline lwt_long_type<(static_cast<wchar_t>(-1) < static_cast<wchar_t>(0))>::type
test_output_impl(wchar_t const& v)
{
	return v;
}
...
template <class T>
char const*
lwt_predicate_name(T const&)
{
	return "~=";
}

inline char const*
lwt_predicate_name(lw_test_eq const&)
{
	return "==";
}

inline char const*
lwt_predicate_name(lw_test_ne const&)
{
	return "!=";
}

inline char const*
lwt_predicate_name(lw_test_lt const&)
{
	return "<";
}

inline char const*
lwt_predicate_name(lw_test_le const&)
{
	return "<=";
}

inline char const*
lwt_predicate_name(lw_test_gt const&)
{
	return ">";
}

inline char const*
lwt_predicate_name(lw_test_ge const&)
{
	return ">=";
}
    ...
    U const& u)
{
	...
	{
		BOOST_LIGHTWEIGHT_TEST_OSTREAM
		    << file << "(" << line << "): test '" << expr1 << " " << lwt_predicate_name(pred)
...
bool
test_all_eq_impl(
    FormattedOutputFunction& output,
    char const* file,
    int line,
    char const* function,
    InputIterator1 first_begin,
    ...
    InputIterator2 second_end)
{
	...
	do {
		while (
		    (first_it != first_end) && (second_it != second_end) && (*first_it == *second_it))
		{
			++first_it;
			++second_it;
			++first_index;
		...
		if (error_count == 0)
		{
			output << file << "(" << line << "): Container contents differ in function '"
			       << function << "':";
		}
	...
	if (first_index != second_index)
	{
		...
		{
			output
			    << file << "(" << line << "): Container sizes differ in function '" << function
			    << "': size(" << first_index << ") != size(" << second_index << ")";
		}
		else { output << " [*] size(" << first_index << ") != size(" << second_index << ")"; }
		++error_count;
	}
...
#define BOOST_TEST(expr)                                                                      \
	(::boost::detail::test_impl(                                                              \
	    #expr, __FILE__, __LINE__, BOOST_CURRENT_FUNCTION, (expr) ? true : false))
...
#define BOOST_TEST_WITH(expr1, expr2, predicate)                                              \
	(::boost::detail::test_with_impl(                                                         \
	    predicate, #expr1, #expr2, __FILE__, __LINE__, BOOST_CURRENT_FUNCTION, expr1, expr2))

#define BOOST_TEST_EQ(expr1, expr2)                                                           \
	(::boost::detail::test_with_impl(                                                         \
	    ::boost::detail::lw_test_eq(), #expr1, #expr2, __FILE__, __LINE__,                    \
	    BOOST_CURRENT_FUNCTION, expr1, expr2))
#define BOOST_TEST_NE(expr1, expr2)                                                           \
	(::boost::detail::test_with_impl(                                                         \
	    ::boost::detail::lw_test_ne(), #expr1, #expr2, __FILE__, __LINE__,                    \
	    BOOST_CURRENT_FUNCTION, expr1, expr2))

#define BOOST_TEST_LT(expr1, expr2)                                                           \
	(::boost::detail::test_with_impl(                                                         \
	    ::boost::detail::lw_test_lt(), #expr1, #expr2, __FILE__, __LINE__,                    \
	    BOOST_CURRENT_FUNCTION, expr1, expr2))
#define BOOST_TEST_LE(expr1, expr2)                                                           \
	(::boost::detail::test_with_impl(                                                         \
	    ::boost::detail::lw_test_le(), #expr1, #expr2, __FILE__, __LINE__,                    \
	    BOOST_CURRENT_FUNCTION, expr1, expr2))
#define BOOST_TEST_GT(expr1, expr2)                                                           \
	(::boost::detail::test_with_impl(                                                         \
	    ::boost::detail::lw_test_gt(), #expr1, #expr2, __FILE__, __LINE__,                    \
	    BOOST_CURRENT_FUNCTION, expr1, expr2))
#define BOOST_TEST_GE(expr1, expr2)                                                           \
	(::boost::detail::test_with_impl(                                                         \
	    ::boost::detail::lw_test_ge(), #expr1, #expr2, __FILE__, __LINE__,                    \
	    BOOST_CURRENT_FUNCTION, expr1, expr2))

#define BOOST_TEST_CSTR_EQ(expr1, expr2)                                                      \
	(::boost::detail::test_cstr_eq_impl(                                                      \
	    #expr1, #expr2, __FILE__, __LINE__, BOOST_CURRENT_FUNCTION, expr1, expr2))
#define BOOST_TEST_CSTR_NE(expr1, expr2)                                                      \
	(::boost::detail::test_cstr_ne_impl(                                                      \
	    #expr1, #expr2, __FILE__, __LINE__, BOOST_CURRENT_FUNCTION, expr1, expr2))

#define BOOST_TEST_ALL_EQ(begin1, end1, begin2, end2)                                         \
	(::boost::detail::test_all_eq_impl(                                                       \
	    BOOST_LIGHTWEIGHT_TEST_OSTREAM, __FILE__, __LINE__, BOOST_CURRENT_FUNCTION, begin1,   \
	    end1, begin2, end2))
#define BOOST_TEST_ALL_WITH(begin1, end1, begin2, end2, predicate)                            \
	(::boost::detail::test_all_with_impl(                                                     \
	    BOOST_LIGHTWEIGHT_TEST_OSTREAM, __FILE__, __LINE__, BOOST_CURRENT_FUNCTION, begin1,   \
	    end1, begin2, end2, predicate))
