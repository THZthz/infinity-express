...
inline void
intrusive_ptr_add_ref(N::base const *p)
{
	p->add_ref();
}

inline void
intrusive_ptr_release(N::base const *p)
{
	p->release();
}
...
void
test2(boost::intrusive_ptr<T> const &p, boost::intrusive_ptr<U> const &q)
{
...
template <class T>
void
test3(boost::intrusive_ptr<T> const &p, boost::intrusive_ptr<T> const &q)
{
	BOOST_TEST((p == q) == (p.get() == q.get()));
...
test()
{
	...
	{
		boost::intrusive_ptr<Y> py =
		    boost::dynamic_pointer_cast<Y>(boost::intrusive_ptr<X>(static_cast<X *>(0)));
		BOOST_TEST(py.get() == 0);
	}
