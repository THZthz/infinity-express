...
public:
	explicit X(int expected, boost::shared_ptr<X>* early_px = 0)
	...
	static void deleter(X* px) { ++px->deleted_; }
...
bool
are_shared_owners(const boost::shared_ptr<T>& a, const boost::shared_ptr<U>& b)
{
	return !(a < b) && !(b < a);
}
